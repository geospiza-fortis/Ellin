    ...
    if (mode == -1) {
        ...
        if (status == 0) {
            if (qm.getPlayer().getHp() <= 1) {
                qm.sendNext(
                    "I found Jack Barricade resting in a cave on Crimsonwood Mountain. It turns out that after exploring the Masterian wilderness for weeks, Jack finally came across the ancient stronghold - only it wasn't deserted."
                );
                qm.dispose();
            } else {
                qm.sendNext(
                    "Jack was captured and imprisoned in the Keep by the evil soldiers I've been seeing around, but luckily managed to escape. John will be relieved that Jack is okay."
                );
            }
        } else if (status == 1) {
            qm.sendNextPrev(
                "I found Jack Barricade resting in a cave on Crimsonwood Mountain. It turns out that after exploring the Masterian wilderness for weeks, Jack finally came across the ancient stronghold - only it wasn't deserted. Jack was captured and imprisoned in the Keep by the evil soldiers I've been seeing around, but luckily managed to escape. John will be relieved that Jack is okay."
            );
        } else if (status == 2) {
            //qm.gainExp(22500);
            //qm.gainMeso(90000);
