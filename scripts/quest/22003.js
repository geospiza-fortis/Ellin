...
function start(mode, type, selection) {
    ...
    if (status == 0) {
        qm.sendAcceptDecline(
            "Your #bDad#k forgot his Lunch Box when he left for the farm this morning. Will you #bdeliver this Lunch Box#k to your Dad in #b#m100030300##k, honey?"
        );
    } else if (status == 1) {
        if (mode == 0 && type == 15) {
            //decline
            qm.sendNext(
                "Good kids listen to their mothers. Now, Evan, be a good kid and talk to me again."
            );
            qm.dispose();
        } else {
