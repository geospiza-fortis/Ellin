...
function start(mode, type, selection) {
    ...
    if (status == 0) {
        qm.askAcceptDecline(
            "O.(Shivering) I...was...so...scared...in...here. Please take me to Athena Pierce!"
        );
    } else if (status == 1) {
...
function end(mode, type, selection) {
    if (mode == 1) {
        ...
        if (status == 0) {
            qm.sendNext(
                "Where's the kid? If you brought the kid with you, hand him over to me!"
            );
            qm.dispose();
            return;
        } else if (status == 8) {
    ...
    if (status == 0) {
        ...
        qm.sendNextPrevS(
            "Athena Pierce, you take care of these people and head over to Victoria Island. I promise you, I will not die. I will meet you there at the island soon. I better help my friends out and battle the Black Wizard once and for all!",
            3
        );
    } else if (status == 8) {
        qm.sendYesNo(
