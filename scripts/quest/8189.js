...
function end(mode, type, selection) {
    if (mode == -1) {
        ...
        if (status == 0) {
            qm.sendYesNo(
                "Alright then, let's do this again, shall we? As usual, it's going to be random, and I'm going to take away one of your Rock of Evolutions. \r\n\r #r#eReady?#n#k"
            );
        } else if (status == 1) {
            ...
            var pet = 0;
            if (
                qm.getPlayer().getPet(0).getItemId() >= 5000029 &&
                qm.getPlayer().getPet(0).getItemId() <= 5000033
            ) {
                var pet = 0;
            } else if (
                qm.getPlayer().getPet(1).getItemId() >= 5000029 &&
                qm.getPlayer().getPet(0).getItemId() <= 5000033
            ) {
                var pet = 1;
            } else if (
                qm.getPlayer().getPet(2).getItemId() >= 5000029 &&
                qm.getPlayer().getPet(0).getItemId() <= 5000033
            ) {
                var pet = 2;
            } else {
            ...
            qm.sendOk(
                "Woo! It worked again! #rYou may find your new pet under your 'CASH' inventory.\r It used to be a #i" +
                    id +
                    "##t" +
                    id +
                    "#, and now it's \r a #i" +
