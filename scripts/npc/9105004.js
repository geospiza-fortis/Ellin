...
function action(mode, type, selection) {
    ...
    switch (cm.getPlayer().getMapId()) {
        case 889100100:
            if (status == 0) {
                cm.sendSimple(
                    "Ol�, eu sou o Esp�rito da Neve.#b\r\n\r\n#L0#Ir Proteger Snowman - F�cil (Lvl. 10)#l\r\n#L1#Ir Proteger Snowman - M�dio (Lvl. 30)#l\r\n#L2#Ir Proteger Snowman - Dif�cil (Lvl. 70)#l"
                );
            } else if (status == 1) {
                if (selection == 0) {
                    if (
                        (cm.getPlayer().getLevel() > 9 &&
                            cm.getPlayer().getLevel() < 31) ||
                        cm.getPlayer().isGM()
                    ) {
                        cm.warp(889100000, 0);
                        cm.dispose();
                    ...
                    if (
                        (cm.getPlayer().getLevel() > 29 &&
                            cm.getPlayer().getLevel() < 71) ||
                        cm.getPlayer().isGM()
                    ) {
                        cm.warp(889100010, 0);
                        cm.dispose();
            ...
            if (status == 0) {
                ...
                if (cm.getPlayer().getParty() == null || !cm.isLeader()) {
                    ...
                    if (next && size >= 1) {
                        ...
                        cm.sendOk(
                            "Todos os membros de seu grupo devem estar aqui."
                        );
                    }
                }
                cm.dispose();
        ...
        case 889100021:
            if (cm.getPlayer().getEventInstance() == null || !cm.isLeader()) {
                ...
                if (
                    ...
                    cm.sendOk(
                        "Por favor, proteja o boneco de neve at� que o tempo se esgote!"
                    );
                } else if (
                    cm.getPlayer().getMap().countMonster(9400319) > 0 ||
                    cm.getPlayer().getMap().countMonster(9400320) > 0 ||
                    cm.getPlayer().getMap().countMonster(9400321) > 0
                ) {
