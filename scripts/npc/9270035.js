...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        sel = selection;
        if (selection == 0) cm.listMonsters();
        else if (selection == 1)
            cm.sendGetText(
                "Digite o nome do monstro que voc� deseja encontrar os drops."
            );
        else if (selection == 2) cm.sendGetText("Coloque o ID do item abaixo:");
        else if (selection == 3)
