...
function processNanaQuest() {
    if (cm.haveItem(questItems[nanaLoc], 50)) {
        if (cm.canHold(4031367 + nanaLoc, 1)) {
            ...
            cm.sendOk(
                "Please have a free ETC slot available to hold the token of love."
            );
        }
    } else {
            ...
            if (!cm.haveItem(4031367 + nanaLoc, 1)) {
                if (cm.isQuestCompleted(100401 + nanaLoc)) {
                    ...
                    cm.sendAcceptDecline(
                        "Did you lost the #k#t4031367##k I gave to you? Well, I can share another one with you, but you will need to redo the favor I asked last time, is that ok? I need you to bring me #r50 #t" +
                            questItems[nanaLoc] +
                            "#'s.#k"
                    );
                    ...
                    state = 0;
                    cm.sendAcceptDecline(
                        "Are you searching for #k#t4031367#'s#k? I can share one with you, but you must do a favor for me, is that ok?"
                    );
                }
            } else {
                cm.sendOk(
                    "Hey there. Did you get the #t4031367# from the other Nana's already?"
                );
                cm.dispose();
            }
        } else if (status == 1) {
