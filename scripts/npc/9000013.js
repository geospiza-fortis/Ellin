...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (status == 0) {
            cm.sendNext(
                "Hi, I'm #bTony#k. I've been waiting for my brothers for a while, but they haven't gotten here yet. I'm sick and tired of doing things by myself. At least during the event, I don't feel quite so lonely with so many people around me and all. All events require a limited number of people, so if I don't get there fast enough, I won't be able to join."
            );
        } else if (status == 1) {
            cm.sendNextPrev(
                "We may be cousins and all, but we keep on missing each other. Man, what should I do? The event should start any minute... A lot of people should be there waiting, and there may not be any space for them..."
            );
        } else if (status == 2) {
            cm.sendSimple(
                "What do you think? Do you want to join me and head over to the event?\r\n#L0##e1.#n#b What kind of an event is it?#k#l\r\n#L1##e2.#n#b Explain the event game to me.#k#l\r\n#L2##e3.#n#b Alright, let's go!#k#l"
            );
        } else if (status == 3) {
            if (selection == 0) {
                cm.sendNext(
