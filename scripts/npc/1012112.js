        ...
        if (cm.getMapId() == 100000200) {
            if (status == 0) {
                ...
                if (selection == 0) {
                    if (cm.getParty() == null) {
                        cm.sendOk(
                            "Hi there! I'm Tory. This place is covered with mysterious aura of the full moon, and no one person can enter here by him/herself."
                        );
                        cm.dispose();
                    } else if (!cm.isLeader()) {
                        cm.sendOk(
                            "If you'd like to enter here, the leader of your party will have to talk to me. Talk to your party leader about this."
                        );
                        cm.dispose();
                    } else {
                        ...
                        if (eli.size() > 0) {
                            if (
                                ...
                                cm.sendOk(
                                    "Someone is already attempting the PQ. Please wait for them to finish, or find another channel."
                                );
                            }
                        } else {
                            cm.sendOk(
            ...
            if (status == 0) {
                ...
                if (cm.getEventInstance().giveEventReward(cm.getPlayer())) {
                    ...
                    cm.sendOk(
                        "It seems you are short on space in one of your inventories. Please check that first to get rewarded properly."
                    );
                }
                cm.dispose();
            }
