        ...
        if (cm.getPlayer().getMapId() != 800000000) {
            if (status == 0) {
                if (cm.getJob().equals(Packages.client.MapleJob.BEGINNER)) {
                    cm.sendNext(
                        "Se voce esta cansado do cotidiano monotono, que tal sair para uma mudanca? Nao ha nada como a absorver uma nova cultura, aprender algo novo a cada minuto! E hora de voce sair e viajar. Recomendamos uma #bWorld Tour#k, voce esta preocupado com a despesa de viagem? Nao precisa se preocupar! O #bMaple Travel Agency#k oferece o alojamento em viagens de primeira classe para o baixo preco de #b300 mesos#k."
                    );
                    cost = 300;
                } else {
                    cm.sendNext(
            ...
            if (status == 0) {
                cm.sendSimple(
                    "How's the traveling? Are you enjoying it?\r\n#L0##bYes, I'm done with travelling. Can I go back to #m" +
                        cm
                            .getPlayer()
                            .getSavedLocation(SavedLocationType.WORLDTOUR) +
                ...
                if (selection == 0) {
                    ...
                    cm.sendOk(
                        "OK. If you ever change your mind, please let me know."
                    );
                    cm.dispose();
                }
            } else if (status == 2) {
