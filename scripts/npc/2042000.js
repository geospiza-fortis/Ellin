...
function start() {
    if (cm.getMapId() != 980000000) {
        MCTracker.log(
            "Spiegelmann called on invalid map " +
                cm.getMapId() +
                " by player " +
                cm.getName()
        );
...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        if (cm.getParty() == null) {
            cm.sendOk(
                "You are not affiliated with any party here. You can only take on this challenge if you are in a party."
            );
            cm.dispose();
            return;
        } else if (!cm.isLeader()) {
            cm.sendOk(
                "If you want to try Carnival PQ, please tell the #bleader of your party#k to talk to me."
            );
            cm.dispose();
            return;
        }
        carnival = MonsterCarnival.getMonsterCarnival(
            cm.getPlayer().getClient().getChannel()
        );
        ...
        if (
            ...
            cm.sendYesNo(
                "Do you want to create a #bCarnival Field#k? Once the Carnival Field is open, you may accept invitations from other parties for #b3 minutes#k. Once you accept an invitation, the opposing party will automatically enter the Carnival Field."
            );
        }
        selected = selection;
    } else if (status == 2) {
        ...
        if (code == MonsterCarnival.STATUS_FIELD_FULL) {
            ...
            cm.sendOk(
                "Your size of the party does not meet the requirements for this quest."
            );
        } else if (code == MonsterCarnival.STATUS_PARTY_LEVEL) {
            cm.sendOk(
                "One of the members in your party may not be Level 30~50. Please alter your party to meet the requirements."
            );
        } else if (code == MonsterCarnival.STATUS_PARTY_MISSING) {
            cm.sendOk(
                "Please make sure everyone in your party is in this lobby."
            );
        ...
        if (code == MonsterCarnival.STATUS_PROCEED) {
            ...
            cm.getPlayer().dropMessage(
                6,
                "You will receive invitations from other parties for the next 3 minutes."
            );
