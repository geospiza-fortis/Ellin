    ...
    if (mode == -1) {
        ...
        if (eim == null) {
            ...
            if (cm.isEventLeader()) {
                if (status == 0) {
                    if (
                        eim.getProperty("stage1status") == null ||
                        eim.getProperty("stage1status").equals("waiting")
                    ) {
                        ...
                        eim.getProperty("stage1status").equals("active")
                    ) {
                        stage = parseInt(eim.getProperty("stage1phase"));

                        if (
                            eim
                                .getProperty("stage1combo")
                                .equals(eim.getProperty("stage1guess"))
                        ) {
                            if (stage == 3) {
                                cm.getPlayer()
                                    .getMap()
                                    .getReactorByName("statuegate")
                                    .forceHitReactor(1);
                                clearStage(1, eim);
                                ...
                                cm.sendOk(
                                    "Very good. You still have more to complete, however. Talk to me again when you're ready."
                                );
                                eim.setProperty("stage1phase", stage + 1);
                                cm.mapMessage(
                                    5,
                                    "You have completed part " +
                                        stage +
                                        " of the Gatekeeper Test."
                                );
                            }
                        } else {
                            eim.showWrongEffect();
                            ...
                            cm.mapMessage(
                                5,
                                "You have failed the Gatekeeper Test."
                            );
                        ...
                        cm.sendOk(
                            "The statues are working on the pattern. Please wait."
                        );
                        cm.dispose();
                    }
                } else if (status == 1) {
                    ...
                    cm.mapMessage(
                        5,
                        "Please wait while the combination is revealed."
                    );
                    var delay = 5000;
                    for (var i = 0; i < combo.length; i++) {
                        cm.getPlayer()
                            .getMap()
                            .getReactorByOid(combo[i])
                            .delayedHitReactor(
                                cm.getClient(),
                                delay + 3500 * i
                            );
                ...
                cm.sendOk(
                    "I need the leader of this event to speak with me, nobody else."
                );
                cm.dispose();
            }
        }
