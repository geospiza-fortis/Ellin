...
public class PartyHandler {
        ...
        if (party == null) {
            ...
            if (party != null) {
                if (party.getMembers().size() < 6) {
                    MaplePartyCharacter partyPlayer = new MaplePartyCharacter(
                        p
                    );
                    PartyService.updateParty(
                        party.getId(),
                        MaplePartyOperation.JOIN,
                        partyPlayer
                    );
                ...
                p.announce(
                    PacketCreator.ServerNotice(
                        5,
                        "The party you are trying to enter does not exist."
                    )
                );
            }
        } else {
    ...
    public static void InvitePartyPlayer(PacketReader packet, Player p) {
        ...
        if (party.getMembers().size() < 6) {
            invited
                .getClient()
                .getSession()
                .write(PartyPackets.PartyInvite(p.getPartyId(), p.getName()));
        } else {
            p.announce(PartyPackets.PartyStatusMessage(PARTY_FULL));
        }
    }
    ...
    public static void ChangePartyLeader(PacketReader packet, Player p) {
        ...
        final Player cfrom = p
            .getClient()
            .getChannelServer()
            .getPlayerStorage()
            .getCharacterById(newLeaderID);
        ...
        if (newLeadr != null && cfrom.getMapId() == p.getMapId()) {
            ...
            p.dropMessage(
                5,
                "The Party Leader can only be handed over to the party member in the same map."
            );
        }
    }
    ...
    public static void PartyResponse(PacketReader packet, Client c) {
        ...
        if (cfrom != null) {
            cfrom
                .getClient()
                .getSession()
                .write(PartyPackets.PartyInviteRejected(to));
        }
    }
}
