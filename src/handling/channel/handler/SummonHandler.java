...
public class SummonHandler {
    ...
    public static void SummonAttack(PacketReader packet, Client c) {
        ...
        PlayerSkill summonSkill = PlayerSkillFactory.getSkill(
            summon.getSkill()
        );
        MapleStatEffect summonEffect = summonSkill.getEffect(
            summon.getSkillLevel()
        );
        final PlayerSummonSkillEntry sse = PlayerSkillFactory.getSummonData(
            summon.getSkill()
        );
        if (
            summon.getSkill() / 1000000 != 35 &&
        ...
        if (sse != null && numAttacked > sse.mobCount) {
            p
                .getCheatTracker()
                .registerOffense(CheatingOffense.SUMMON_HACK_MOBS);
        }
        ...
        allDamage.forEach(
            attackEntry -> {
                ...
                MapleMonster target = p
                    .getMap()
                    .getMonsterByOid(attackEntry.getMonster().getObjectId());
                if (target != null) {
                    if (
                        damage > 0 && summonEffect.getMonsterStati().size() > 0
                    ) {
