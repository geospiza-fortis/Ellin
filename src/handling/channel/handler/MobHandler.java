...
public class MobHandler {
    ...
    public static void MoveMonster(PacketReader packet, Client c) {
        ...
        if (skillByte == 1 && monster.getNoSkills() > 0) {
            ...
            int percHpLeft = (int) (
                (monster.getHp() / monster.getMaxHp()) * 100
            );
        ...
        if (
            skillOne >= 100 &&
            skillOne <= 200 &&
            monster.hasSkill(skillOne, skillTwo)
        ) {
            MobSkill skillData = MobSkillFactory.getMobSkill(
                skillOne,
                skillTwo
            );
        ...
        if (toUse != null) {
            c
                .getSession()
                .write(
                    MonsterPackets.MoveMonsterResponse(
                        objectid,
                        moveid,
                        monster.getMp(),
                        aggro,
        ...
        if (monster != null && monster.getController() != null) {
            if (!monster.controllerHasAggro()) {
                if (
                    p
                        .getMap()
                        .getCharacterById(monster.getController().getId()) ==
                    null
                ) {
    ...
    public static void MonsterBomb(PacketReader slea, Client c) {
        ...
        if (monster.getStats().selfDestruction().getAction() == 2) {
            monster
                .getMap()
                .broadcastMessage(
                    MonsterPackets.KillMonster(monster.getObjectId(), 4)
                );
    ...
    public static void FriendlyDamage(PacketReader packet, Client c) {
        MapleMonster attacker = c
            .getPlayer()
            .getMap()
            .getMonsterByOid(packet.readInt());
        packet.readInt();
        MapleMonster attacked = c
            .getPlayer()
            .getMap()
            .getMonsterByOid(packet.readInt());
        ...
        if (
            (attacker != null) &&
            (attacked != null) &&
            (attacked.getStats().isFriendly())
        ) {
            int damage =
                attacker.getStats().getPADamage() +
                attacker.getStats().getPDDamage() -
                1;
            ...
            if (attacked.getHp() - damage < 1) {
                ...
                if (attacked.getId() == 9300061) {
                    Field map = c
                        .getPlayer()
                        .getEventInstance()
                        .getMapInstance(attacked.getMap().getId());
                    map.addBunnyHit();
                }
            }
            ...
            c
                ...
                .getMap()
                .broadcastMessage(
                    MonsterPackets.MobDamageMob(attacked, damage, true)
                );
    ...
    public static void GrenadeEffect(PacketReader packet, Client c) {
        ...
        if (
            PlayerSkillFactory.getSkill(skillId) != null ||
            c.getPlayer().getSkillLevel(skillId) > 0
        ) {
            p
                .getMap()
                .broadcastMessage(
                    p,
                    PacketCreator.ThrowGrenade(
                        p.getId(),
                        position,
                        keyDown,
                        Gunslinger.Grenade,
                        p.getSkillLevel(Gunslinger.Grenade)
                    ),
                    position
                );
        }
