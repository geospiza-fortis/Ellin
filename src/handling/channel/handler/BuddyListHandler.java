...
public class BuddyListHandler {
    ...
    public static void BuddyOperation(
        ...
        switch (packet.readByte()) {
            case BUDDY_INVITE_MODIFY:
                ...
                boolean isOnPending = BuddyService.isBuddyPending(
                    new MapleBuddyInvitedEntry(addName, c.getPlayer().getId())
                );
                if (isOnPending) {
                    c
                        .getSession()
                        .write(
                            PacketCreator.BuddylistMessage(
                ...
                if (buddylist.isFull()) {
                    c
                        .getSession()
                        .write(PacketCreator.BuddylistMessage(YOUR_LIST_FULL));
                    return;
                }
                ...
                if (channel <= 0) {
                    c
                        ...
                        .dropMessage(
                            5,
                            "The character is not registered or not in game."
                        );
                    c.getSession().write(PacketCreator.EnableActions());
                    return;
                }
                final Player otherChar = ChannelServer
                    .getInstance(channel)
                    .getPlayerStorage()
                ...
                if (!otherChar.isGameMaster() || c.getPlayer().isGameMaster()) {
                    if (otherChar.getBuddylist().isFull()) {
                        c
                            .getSession()
                            .write(
                                PacketCreator.BuddylistMessage(THEIR_LIST_FULL)
                            );
                        return;
                    }
                    final BuddyAddResult buddyAddResult = BuddyService.requestBuddyAdd(
                        addName,
                        c.getPlayer()
                    );
                    ...
                    if (buddyAddResult == BuddyAddResult.ALREADY_ON_LIST) {
                        c
                            ...
                            .dropMessage(
                                5,
                                "An error has occured. Please ask the player to re-add you as a buddy again."
                            );
                        c.getSession().write(PacketCreator.EnableActions());
                        return;
                    }
                    ...
                    c
                        ...
                        .dropMessage(
                            ...
                            "You have invited '" +
                            addName +
                            "' into your Buddy List."
                        );
                    c.getSession().write(PacketCreator.EnableActions());
                } else {
                    c
                        .getSession()
                        .write(PacketCreator.BuddylistMessage(NO_GM_INVITES));
                }
                break;
            case BUDDY_ACCEPT:
                final int otherCid = packet.readInt();
                boolean isOnPending_ = BuddyService.isBuddyPending(
                ...
                if (!isOnPending_) {
                    c
                        .getSession()
                        .write(PacketCreator.BuddylistMessage(DENY_ERROR));
                    return;
                }
                final Pair<BuddyAddResult, String> bal = BuddyService.acceptToInvite(
                    c.getPlayer(),
                    otherCid
                );
            ...
            case BUDDY_DELETE_DENY:
                final int otherCID = packet.readInt();
                boolean isInvited = BuddyService.isBuddyPending(
                    new MapleBuddyInvitedEntry(
                        c.getPlayer().getName(),
                        otherCID
                    )
                ...
                if (isInvited) {
                    c
                        ...
                        .dropMessage(
                            5,
                            BuddyService.denyToInvite(c.getPlayer(), otherCID)
                        );
                    c
                        .getSession()
                        .write(
                ...
                if (blz == null) {
                    c
                        ...
                        .dropMessage(
                            5,
                            "An error has occured. The character is not on your buddy list."
                        );
                    c.getSession().write(PacketCreator.EnableActions());
                    return;
                }
                final MapleBuddyList.BuddyDelResult bdr = BuddyService.DeleteBuddy(
                    c.getPlayer(),
                    otherCID
                );
                ...
                if (bdr == MapleBuddyList.BuddyDelResult.IN_CASH_SHOP) {
                    c
                        ...
                        .dropMessage(
                            5,
                            "The character is currently in the Cash Shop."
                        );
                    c.getSession().write(PacketCreator.EnableActions());
                    return;
                }
                ...
                c
                    ...
                    .dropMessage(
                        5,
                        "Your buddy relationship with the deleted person has ended."
                    );
                c.getSession().write(PacketCreator.EnableActions());
                break;
            default:
                System.out.println(
                    "Unknown buddylist action: " + packet.toString()
                );
                break;
        }
    }
