...
public class InterServerHandler {
    ...
    public static void ChangeChannel(PacketReader packet, Client c) {
        ...
        if (
            ...
            FieldLimit.CHANGECHANNEL.check(p.getMap().getFieldLimit())
        ) {
    ...
    public static final void Loggedin(PacketReader packet, final Client c)
        throws SQLException {
        ...
        if (
            ...
            allowLogin =
                !World.isCharacterListConnected(
                    c.loadCharacterNames(c.getWorld())
                );
        }
        ...
        try {
            ...
            p.giveCoolDowns(
                PlayerBuffStorage.getCooldownsFromStorage(p.getId())
            );
            p.giveSilentDebuff(
                PlayerBuffStorage.getDiseaseFromStorage(p.getId())
            );
            ...
            final CharacterIdChannelPair[] onlineBuddies = FindService.multiBuddyFind(
                p.getId(),
                buddyIds
            );
            for (CharacterIdChannelPair onlineBuddy : onlineBuddies) {
                final MapleBuddyListEntry ble = p
                    .getBuddylist()
            ...
            if (messenger != null) {
                MessengerService.silentJoinMessenger(
                    messenger.getId(),
            ...
            if (p.getGuildId() > 0) {
                ...
                if (gs != null) {
                    final List<OutPacket> packetList = AllianceService.getAllianceInfo(
                        gs.getAllianceId(),
                        true
                    );
                    if (packetList != null) {
        ...
        //
        if (
            c.getPlayer().getMapId() == 0 &&
            c.getPlayer().getLevel() == 1 &&
            ServerProperties.Misc.WELCOME_MESSAGE
        ) {
            c
                .getChannelServer()
                .broadcastYellowMessage(
                    "[Welcome] The player <" +
                    c.getPlayer().getName() +
                    "> acabou de se juntar ao nosso servidor!"
                );
            c.announce(
                PacketCreator.GetWhisper(
                    "[" + ServerProperties.Login.SERVER_NAME + "]",
                    1,
                    "Hello mapler, use the @help command and have a good game."
                )
            );
        }
        if (ServerProperties.Misc.VOTE_MESSAGE) {
            if (!c.hasVotedAlready()) {
                p.dropMessage(
                    "Hello, have not you \"voted\" today? How about voting and receiving rewards?"
                );
                p.dropMessage(
                    "Go to " +
                    ServerProperties.Misc.WEB_SITE +
                    " and click on \"Vote\"!"
                );
            }
        }
    ...
    public static void EnterCS(PacketReader packet, Client c) {
        try {
            Player p = c.getPlayer();
            if (
                !ServerProperties.Misc.CASHSHOP_AVAILABLE ||
                p.getCashShop().isOpened()
            ) {
            ...
            c.announce(
                CashShopPackets.GiftedCashItems(p.getCashShop().loadGifts(c))
            );
