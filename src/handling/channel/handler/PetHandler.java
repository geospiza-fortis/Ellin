...
public class PetHandler {
    ...
    public static void MovePet(PacketReader packet, Client c) {
        ...
        if (mov != null && p != null && !mov.isEmpty()) {
            ...
            p
                ...
                .broadcastMessage(
                    p,
                    PetPackets.MovePet(p.getId(), petId, slot, mov),
                    false
                );
        }
    }
    ...
    public static void PetChat(PacketReader packet, Client c) {
        ...
        if (p.getPetIndex(petId) == -1 || p.getPetIndex(petId) > 3) {
            p
                ...
                .registerOffense(
                    ...
                    "Tried to pet chat with nonexistent pet"
                );
            return;
        }
        ...
        if (action < 1 || ItemPetFactory.IsValidPetAction(petItemId, action)) {
            p
                ...
                .registerOffense(
                    CheatingOffense.PACKET_EDIT,
                    "Tried to pet chat with nonexistent action {" + action + "}"
                );
            return;
        }
        ...
        if (message.length() > Byte.MAX_VALUE) {
            AutobanManager
                ...
                .autoban(
                    p.getClient(),
                    p.getName() + " tried to packet edit with pets."
                );
            return;
        }
        p
            ...
            .broadcastMessage(
                ...
                PetPackets.PetChat(
                    ...
                    message,
                    p.getPetIndex(petId),
                    c.getPlayer().haveItem(1832000, 1, true, false)
                ),
                true
            );
    ...
    public static void PetExcludeItems(PacketReader packet, Client c) {
        ...
        if (p.getPetIndex(uniqueId) == -1) {
            p
                ...
                .registerOffense(
                    CheatingOffense.PACKET_EDIT,
                    "Tried to use pet item ignore with nonexistent pet"
                );
            return;
        }
    ...
    public static void PetCommand(PacketReader packet, Client c) {
        final byte petIndex = c
            .getPlayer()
            .getPetIndex((int) packet.readLong());
        if (petIndex == -1) {
            c
                .getPlayer()
                .getCheatTracker()
                .registerOffense(
                    CheatingOffense.PACKET_EDIT,
                    "Tried to use pet command with nonexistent pet"
                );
            return;
        }
        ...
        final ItemPetCommand petCommand = ItemPetFactory.getPetCommand(
            pet.getPetItemId(),
            (int) command
        );
        if (petCommand == null) {
            c
                ...
                .registerOffense(
                    CheatingOffense.PACKET_EDIT,
                    "Tried to use nonexistent pet command"
                );
            return;
        }
        ...
        if (Randomizer.nextInt(99) <= petCommand.getProbability()) {
            ...
            if (pet.getCloseness() < 30000) {
                int newCloseness =
                    pet.getCloseness() + petCommand.getIncrease();
                ...
                if (
                    ...
                    c
                        .getSession()
                        .write(PetPackets.ShowOwnPetLevelUp(petIndex));
                    c
                        .getPlayer()
                        .getMap()
                        .broadcastMessage(
                            PetPackets.ShowPetLevelUp(c.getPlayer(), petIndex)
                        );
                }
    ...
    public static void PetFood(PacketReader r, Client c) {
        ...
        Item petFood = c
            ...
            .getItem((byte) foodSlot);
        if (
            petFood == null ||
            petFood.getItemId() != foodItemId ||
            petFood.getQuantity() <= 0 ||
            foodItemId / 10000 != 212
        ) {
            p
                .getCheatTracker()
                .registerOffense(
                    CheatingOffense.PACKET_EDIT,
                    "Tried to use nonexistent pet food"
                );
            return;
        }
            ...
            if (fullness < 100) {
                ...
                if (gainCloseness && closeness < 30000) {
                    ...
                    if (
                        ...
                        c
                            .getSession()
                            .write(PetPackets.ShowOwnPetLevelUp(index));
                        p
                            .getMap()
                            .broadcastMessage(
                                PetPackets.ShowPetLevelUp(c.getPlayer(), index)
                            );
                    }
                }
                c.getSession().write(PetPackets.updatePet(pet, true));
        ...
        try {
            ...
            if (p.getPetIndex(uniqueId) == -1) {
                p
                    ...
                    .registerOffense(
                        CheatingOffense.PACKET_EDIT,
                        "Tried to use pet auto potion with nonexistent pet"
                    );
                return;
            }
            ...
            if (
                ...
                p
                    ...
                    .registerOffense(
                        CheatingOffense.PACKET_EDIT,
                        "Tried to use nonexistent potion for pet auto potion"
                    );
                return;
            }
            if (
                p.getAutoHpPot() != toUse.getItemId() &&
                p.getAutoMpPot() != toUse.getItemId()
            ) {
                p
                    .getCheatTracker()
                    .registerOffense(
                        CheatingOffense.PACKET_EDIT,
                        "Tried to use wrong potion for pet auto potion"
                    );
                return;
            }
            if (
                p.getAutoHpPot() == toUse.getItemId() &&
                !p.haveItemEquiped(ItemConstants.HP_ITEM) ||
                p.getAutoMpPot() == toUse.getItemId() &&
                !p.haveItemEquiped(ItemConstants.MP_ITEM)
            ) {
                p
                    .getCheatTracker()
                    .registerOffense(
                        CheatingOffense.PACKET_EDIT,
                        "Tried to use pet auto potion without equip"
                    );
                return;
            }
