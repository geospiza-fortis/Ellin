...
public class AllianceHandler {
    ...
    private static void AllianceUnknow(Client c, MapleGuild gs) {
        for (OutPacket pack : AllianceService.getAllianceInfo(
            gs.getAllianceId(),
            false
        )) {
            if (pack != null) {
    ...
    private static void AllianceInvite(
        Client c,
        MapleGuild gs,
        PacketReader packet,
        int leaderid
    ) {
        final int newGuild = GuildService.getGuildLeader(
        ...
        if (
            ...
            Player p = c
                .getChannelServer()
                .getPlayerStorage()
                .getCharacterById(newGuild);
            if (
                p != null &&
                p.getGuildId() > 0 &&
                AllianceService.canInvite(gs.getAllianceId())
            ) {
                p
                    .getClient()
                    .getSession()
                    .write(
                        GuildPackets.SendAllianceInvite(
                            AllianceService
    ...
    private static void AllianceAcceptInvite(Client c) {
        ...
        if (inviteid > 0) {
            if (
                !AllianceService.addGuildToAlliance(
                    inviteid,
                    c.getPlayer().getGuildId()
                )
            ) {
                c
                    .getPlayer()
                    .dropMessage(5, "An error occured when adding guild.");
            }
            GuildService.setInvitedId(c.getPlayer().getGuildId(), 0);
        }
    }

    private static void AllianceLeaveAndExpel(
        Client c,
        MapleGuild gs,
        PacketReader packet,
        byte operation
        ...
        if (operation == 6 && packet.available() >= 4) {
            ...
            if (
                packet.available() >= 4 &&
                gs.getAllianceId() != packet.readInt()
            ) {
                return;
            }
        } else {
        ...
        if (
            ...
            if (
                !AllianceService.removeGuildFromAlliance(
                    gs.getAllianceId(),
                    gid,
                    c.getPlayer().getGuildId() != gid
                )
                ...
                c
                    .getPlayer()
                    .dropMessage(5, "An error occured when removing guild.");
            }
        }
    }

    private static void AllianceChangeLeader(
        Client c,
        MapleGuild gs,
        PacketReader packet,
        int leaderid
        ...
        if (
            ...
            if (
                !AllianceService.changeAllianceLeader(
                    gs.getAllianceId(),
                    packet.readInt()
                )
            ) {
                c
                    .getPlayer()
                    .dropMessage(5, "An error occured when changing leader.");
            }
        }
    }

    private static void AllianceTitleUpdate(
        Client c,
        MapleGuild gs,
        PacketReader packet,
        int leaderid
    ...
    private static void AllianceChangeRank(
        Client c,
        MapleGuild gs,
        PacketReader packet
    ) {
        if (c.getPlayer().getAllianceRank() <= 2) {
            if (
                !AllianceService.changeAllianceRank(
                    gs.getAllianceId(),
                    packet.readInt(),
                    packet.readByte()
                )
                ...
                c
                    .getPlayer()
                    .dropMessage(5, "An error occured when changing rank.");
            }
        }
    }

    private static void AllianceNoticeUpdate(
        Client c,
        MapleGuild gs,
        PacketReader packet
    ) {
