...
public class PlayerInteractionHandler {
    ...
    private static boolean cantCreateInteraction(
        Player p,
        byte type,
        boolean creation
    ) {
        ...
        switch (type) {
            ...
            case MODE_MATCH_CARDS:
                if (
                    p.getChalkboard() != null ||
                    FieldLimit.CANNOTMINIGAME.check(p.getMap().getFieldLimit())
                ) {
                    p.announce(PacketCreator.EnableActions());
                    return false;
                }
    ...
    private static void createRoom(Player p, PacketReader packet) {
        ...
        switch (packet.readByte()) {
            case MODE_OMOK:
                {
                    ...
                    p
                        .getMap()
                        .broadcastMessage(
                            MinigamePackets.AddBoxGame(p, 1, 0, true)
                        );
            ...
            case MODE_MATCH_CARDS:
                {
                    if (
                        !cantCreateInteraction(p, MODE_MATCH_CARDS, true)
                    ) return;
                    ...
                    p
                        .getMap()
                        .broadcastMessage(
                            MinigamePackets.AddBoxGame(p, 1, 0, false)
                        );
            ...
            case MODE_PLAYER_SHOP:
                {
                    if (
                        !cantCreateInteraction(p, MODE_PLAYER_SHOP, true)
                    ) return;
                    ...
                    if (
                        p.getInventory(InventoryType.CASH).countById(itemId) < 1
                    ) {
                        p
                            .getCheatTracker()
                            .registerOffense(
            ...
            case MODE_HIRED_MERCHANT:
                {
                    if (
                        !cantCreateInteraction(p, MODE_HIRED_MERCHANT, true)
                    ) return;
                    ...
                    p
                        .getClient()
                        .getChannelServer()
                        .addHiredMerchant(p.getId(), (Merchant) merchant);
                    p
                        .getClient()
                        .getSession()
                        .write(MerchantPackets.GetMerchant(p, merchant, true));
                    break;
                }
        }
    }
            ...
            if (ob instanceof PlayerShop) {
                ...
                if (game.hasFreeSlot() && !game.isVisitor(p)) {
                    ...
                    p
                        .getClient()
                        .getSession()
                        .write(MinigamePackets.GetMiniGameFull());
                }
    ...
    private static void openRoom(Player p, PacketReader packet) {
        ...
        if (shop != null && shop.isOwner(p)) {
            ...
            p
                .getMap()
                .broadcastMessage(MerchantPackets.MerchantSpawn(merchant, 5));
            packet.readByte();
        }
    }
    ...
    private static void gameStart(Player p) {
        ...
        if (game.getGameType().equals("omok")) {
            game.broadcast(
                MinigamePackets.GetMiniGameStart(game, game.getLoser())
            );
            p
                .getMap()
                .broadcastMessage(
                    MinigamePackets.AddBoxGame(game.getOwner(), 2, 1, true)
                );
        }
        if (game.getGameType().equals("matchcard")) {
            game.shuffleList();
            game.broadcast(
                MinigamePackets.GetMatchCardStart(game, game.getLoser())
            );
            p
                .getMap()
                .broadcastMessage(
                    MinigamePackets.AddBoxGame(game.getOwner(), 2, 1, false)
                );
        }
    }
    ...
    private static void gameRequestTie(Player p) {
        Minigame game = p.getMiniGame();
        if (game.isOwner(p)) {
            game.broadcastToVisitor(
                MinigamePackets.GetMiniGameRequestTie(game)
            );
        } else {
            game
                .getOwner()
                .getClient()
                .getSession()
                .write(MinigamePackets.GetMiniGameRequestTie(game));
        }
    }
    ...
    private static void gameSelectCard(Player p, PacketReader packet) {
        ...
        if (turn == 1) {
            ...
            if (game.isOwner(p)) {
                game.broadcastToVisitor(
                    MinigamePackets.GetMatchCardSelect(
                        game,
                        turn,
                        slot,
                        firstslot,
            ...
            (game.getCardId(firstslot + 1)) == (game.getCardId(slot + 1))
        ) {
            if (game.isOwner(p)) {
                game.broadcast(
                    MinigamePackets.GetMatchCardSelect(
        ...
        if (quantity < 1 || quantity > item.getQuantity()) {
            p.announce(
                PacketCreator.ServerNotice(
                    1,
                    "You don't have enough quantity of the item."
                )
            );
            p.announce(PacketCreator.EnableActions());
            return;
    ...
    private static void addItem(Player p, PacketReader packet) {
        ...
        if (
            ...
            !ItemInformationProvider
                .getInstance()
                .isItemValid(ivItem.getItemId())
        ) {
            System.out.println(
                "[REMOVER] Item invalido: " + ivItem.getItemId()
            );
            p.announce(PacketCreator.EnableActions());
            return;
        }
        ...
        if (
            p.getItemQuantity(
                p.getInventory(type).getItem(slot).getItemId(),
                false
            ) <
            bundles
        ) {
        ...
        if (ItemConstants.isRechargeable(ivItem.getItemId())) {
            ...
            p.getItemQuantity(ivItem.getItemId(), false) < perBundle * bundles
        ) {
            p.announce(
                PacketCreator.ServerNotice(
                    1,
                    "Could not perform shop operation with that item."
                )
            );
            p.announce(PacketCreator.EnableActions());
            return;
        ...
        if (
            ...
            AutobanManager
                ...
                .autoban(
                    p.getClient(),
                    p.getName() + " tried to packet edit with hired merchants."
                );
            FileLogger.printError(
                FileLogger.EXPLOITS + p.getName() + ".txt",
                p.getName() +
                " might of possibly packet edited Hired Merchants\nperBundle: " +
                perBundle +
                "\nperBundle * bundles (This multiplied cannot be greater than 2000): " +
                perBundle *
                bundles +
                "\nbundles: " +
                bundles +
        ...
        if (shop != null && shop.isOwner(p)) {
            if (shop.isOpen()) {
                p.announce(
                    PacketCreator.ServerNotice(1, "You can't sell it anymore.")
                );
                return;
            }
            shop.addItem(item);
            ...
            p
                .getClient()
                .getSession()
                .write(MerchantPackets.UpdateMerchant(merchant, p));
        }
        if (ItemConstants.isRechargeable(ivItem.getItemId())) {
            InventoryManipulator.removeFromSlot(
                p.getClient(),
    ...
    private static void buyItem(Player p, PacketReader packet) {
        ...
        if (quantity < 1) {
            AutobanManager
                ...
                .autoban(
                    p.getClient(),
                    p.getName() +
                    " tried to packet edit with a hired merchant and or player shop."
                );
            FileLogger.printError(
                FileLogger.EXPLOITS + p.getName() + ".txt",
                p.getName() +
        ...
        if (shop != null && shop.isVisitor(p)) {
            ...
            merchant.broadcastToVisitorsThreadsafe(
                MerchantPackets.UpdateMerchant(merchant, p)
            );
        }
    }
        ...
        if (merchant != null && merchant.isOwner(p)) {
            if (merchant.isOpen()) {
                p.announce(
                    PacketCreator.ServerNotice(
                        1,
                        "You can't take it with the store open."
                    )
                );
                return;
            }
            ...
            if (slot >= merchant.getItems().size() || slot < 0) {
                AutobanManager
                    ...
                    .autoban(
                        p.getClient(),
                        p.getName() +
                        " tried to packet edit with a hired merchant."
                    );
                FileLogger.printError(
                    FileLogger.EXPLOITS + p.getName() + ".txt",
                    p.getName() +
                    " tried to remove item at slot " +
                    slot +
                    "\r\n"
                );
                return;
            ...
            if (slot >= shop.getItems().size() || slot < 0) {
                AutobanManager
                    ...
                    .autoban(
                        p.getClient(),
                        p.getName() +
                        " tried to packet edit with a player shop."
                    );
                FileLogger.printError(
                    FileLogger.EXPLOITS + p.getName() + ".txt",
                    p.getName() +
    ...
    public static void RockPaperScissors(PacketReader slea, Client c) {
        ...
        switch (mode) {
            ...
            case CONTINUE_RPS:
                if (
                    c.getPlayer().getRPS() == null ||
                    !c.getPlayer().getRPS().nextRound(c)
                ) {
                    c
                        .getSession()
                        .write(
