...
public class PlayersHandler {
    ...
    public static void GiveFame(PacketReader packet, Client c) {
        ...
        final Player receiver = (Player) c
            .getPlayer()
            .getMap()
            .getMapObject(packet.readInt());
        ...
        if (receiver == self) {
            self
                .getCheatTracker()
                .registerOffense(
                    CheatingOffense.FAMING_SELF,
                    "Tried to give fame to self"
                );
            return;
        }
        if (receiver == null) {
            c
                .getSession()
                .write(
                    PacketCreator.GiveFameErrorResponse(
                        FAME_OPERATION_RESPONSE_NOT_IN_MAP
                    )
                );
            return;
        }
        if (self.getLevel() < 15) {
            self
                .getCheatTracker()
                .registerOffense(CheatingOffense.FAMING_UNDER_15);
            c
                .getSession()
                .write(
                    PacketCreator.GiveFameErrorResponse(
                        FAME_OPERATION_RESPONSE_UNDER_LEVEL
                    )
                );
            return;
        }
        if (fameChange != 1 && fameChange != -1) {
            AutobanManager
                ...
                .autoban(
                    self.getClient(),
                    self.getName() + " problaby edit packet."
                );
            return;
        }
        switch (c.getPlayer().canGiveFame(receiver)) {
            case OK:
                if (Math.abs(receiver.getFame() + fameChange) < 30001) {
                    ...
                    receiver
                        .getStat()
                        .updateSingleStat(PlayerStat.FAME, receiver.getFame());
                }
                ...
                c
                    .getSession()
                    .write(
                        PacketCreator.GiveFameResponse(
                            add,
                            receiver.getName(),
                            receiver.getFame()
                        )
                ...
                receiver
                    ...
                    .getSession()
                    .write(
                        PacketCreator.ReceiveFame(add, c.getPlayer().getName())
                    );
                break;
            case NOT_TODAY:
                c
                    ...
                    .write(
                        PacketCreator.GiveFameErrorResponse(
                            FAME_OPEARTION_RESPONSE_NOT_TODAY
                        )
                    );
                break;
            case NOT_THIS_MONTH:
                c
                    ...
                    .write(
                        PacketCreator.GiveFameErrorResponse(
                            FAME_OPERATION_RESPONSE_NOT_THIS_MONTH
                        )
                    );
                break;
        }
            ...
            if (door == null) {
                p
                    ...
                    .registerOffense(
                        ...
                        "Tried to use nonexistent mystic door"
                    );
                return;
            }
        ...
        switch (type) {
            case NOTE_RECEIVE:
                ...
                if (!isPackage) {
                    ...
                    if (
                        item == null ||
                        !item.getGiftFrom().equalsIgnoreCase(name) ||
                        !c
                            .getPlayer()
                            .getCashShop()
                            .canSendNote(item.getUniqueId())
                    ) {
                        return;
                    }
        ...
        switch (mode) {
            case SEND_RING:
                ...
                if (partnerName.equalsIgnoreCase(player.getName())) {
                    player.dropMessage(
                        1,
                        "You cannot put your own name in it."
                    );
                } else if (partner == null) {
                    player.dropMessage(
                        1,
                        partnerName +
                        " was not found on this channel. If you are both logged in, please make sure you are in the same channel."
                    );
                } else if (partner.getGender() == player.getGender()) {
                    player.dropMessage(
                        1,
                        "Your partner is the same gender as you."
                    );
                } else if (
                    player.getPartnerId() > 0 && partner.getPartnerId() > 0
                ) {
                    NPCScriptManager
                        .getInstance()
                        .start(
            ...
            default:
                System.out.println(
                    "Unhandled Ring Packet : " + packet.toString()
                );
                break;
        }
    }
