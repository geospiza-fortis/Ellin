...
public abstract class DamageParse {
    ...
    public static void applyAttack(
        ...
        if (attack.skill != 0) {
            ...
            if (
                PlayerSkillFactory.getSkill(attack.skill).isGMSkill() &&
                !p.isGameMaster()
            ) {
                AutobanManager
                    .getInstance()
                    .autoban(p.getClient(), "Used GMSkills");
                return;
            }
        }
        ...
        if (attackCount > attack.numDamage) {
            if (attack.skill != ChiefBandit.MesoExplosion) {
                p
                    .getCheatTracker()
                    .registerOffense(CheatingOffense.MISMATCHING_BULLETCOUNT);
                return;
            }
        }

        if (
            attackCount != attack.numDamage &&
            attack.skill != ChiefBandit.MesoExplosion &&
            attack.numDamage != attackCount * 2
        ) {
            p
                .getCheatTracker()
                .registerOffense(
                    CheatingOffense.MISMATCHING_BULLETCOUNT,
        ...
        if (attack.skill == ChiefBandit.MesoExplosion) {
            for (Integer oned : attack.allDamage.keySet()) {
                ...
                if (
                    mapObject != null &&
                    mapObject.getType() == FieldObjectType.ITEM
                ) {
                    ...
                    synchronized (mapItem) {
                        if (mapItem.getMeso() > 0) {
                            ...
                            p
                                .getCheatTracker()
                                .registerOffense(CheatingOffense.ETC_EXPLOSION);
                            return;
                        }
                    }
                } else if (
                    mapObject != null &&
                    mapObject.getType() != FieldObjectType.MONSTER
                ) {
                    p
                        .getCheatTracker()
        ...
        for (Integer oned : attack.allDamage.keySet()) {
            ...
            if (monster != null) {
                ...
                if (
                    attack.skill == Bishop.Genesis ||
                    attack.skill == ILArchMage.Blizzard ||
                    attack.skill == FPArchMage.Metoer_Shower
                ) {
                    distanceToDetect += 275000;
                }

                if (
                    attack.skill == Hero.Brandish ||
                    attack.skill == DragonKnight.Spear_Crusher ||
                ...
                if (
                    attack.skill == DragonKnight.DragonRoar ||
                    attack.skill == SuperGm.SuperDragonRoar
                ) {
                ...
                if (attack.skill == Cleric.Heal && !monster.getUndead()) {
                    p
                        .getCheatTracker()
                        .registerOffense(CheatingOffense.HEAL_ATTACKING_UNDEAD);
                    return;
                }
                ...
                if (totDamageToOneMonster > attack.numDamage + 1) {
                    if (
                        ...
                        monster.getId() < 9500317 &&
                        monster.getId() > 9600066
                    ) {
                        p
                            .getCheatTracker()
                            .registerOffense(
                ...
                if (
                    totDamageToOneMonster >= 2499999 &&
                    attack.skill != Marksman.PiercingArrow &&
                    attack.skill != ChiefBandit.MesoExplosion
                ) {
                    AutobanManager
                        .getInstance()
                ...
                if (p.getBuffedValue(BuffStat.PICKPOCKET) != null) {
                    switch (attack.skill) {
                        ...
                        case Shadower.BoomerangStep:
                            final int maxMeso = p.getBuffedValue(
                                BuffStat.PICKPOCKET
                            );
                            final PlayerSkill skillPocket = PlayerSkillFactory.getSkill(
                                ChiefBandit.Pickpocket
                            );
                            final MapleStatEffect s = skillPocket.getEffect(
                                p.getSkillLevel(skillPocket)
                            );
                            ...
                            for (Integer eachd : onedList) {
                                ...
                                if (
                                    skillPocket
                                        .getEffect(p.getSkillLevel(skillPocket))
                                        .makeChanceResult()
                                ) {
                ...
                switch (attack.skill) {
                                ...
                                (
                                    p
                                        ...
                                        .calculateMaxBaseDamage(
                                            p.getStat().getTotalWatk()
                                        ) *
                                    (
                                        PlayerSkillFactory
                                            .getSkill(Paladin.HeavensHammer)
                                            .getEffect(
                                                p.getSkillLevel(
                                                    PlayerSkillFactory.getSkill(
                                                        Paladin.HeavensHammer
                                                    )
                                                )
                                            )
                                            .getDamage() /
                    ...
                    case Marauder.EnergyDrain:
                        int gainHp = (int) (
                            (double) totDamageToOneMonster *
                            (double) PlayerSkillFactory
                                .getSkill(attack.skill)
                                .getEffect(
                                    p.getSkillLevel(
                                        PlayerSkillFactory.getSkill(
                                            attack.skill
                        ...
                        gainHp =
                            Math.min(
                                monster.getMaxHp(),
                                Math.min(gainHp, p.getStat().getMaxHp() / 2)
                            );
                    ...
                    default:
                        ...
                        if (totDamageToOneMonster > 0 && monster.isAlive()) {
                            ...
                            if (p.getJob().isA(PlayerJob.WHITEKNIGHT)) {
                                int[] charges = {
                                    WhiteKnight.SwordIceCharge,
                                    WhiteKnight.BwIceCharge,
                                };
                                for (int charge : charges) {
                                    if (
                                        p.isBuffFrom(
                                            BuffStat.WK_CHARGE,
                                            PlayerSkillFactory.getSkill(charge)
                                        )
                                    ) {
                                        final ElementalEffectiveness iceEffectiveness = monster.getEffectiveness(
                                            Element.ICE
                                        );
                                        if (
                                            iceEffectiveness ==
                ...
                if (
                    p.getSkillLevel(
                        PlayerSkillFactory.getSkill(NightLord.VenomousStar)
                    ) >
                    0
                ) {
                    MapleStatEffect venomEffect = PlayerSkillFactory
                        .getSkill(NightLord.VenomousStar)
                        .getEffect(
                            p.getSkillLevel(
                                PlayerSkillFactory.getSkill(
                                    NightLord.VenomousStar
                                )
                            )
                        );
                    for (int i = 0; i < attackCount; i++) {
                        if (venomEffect.makeChanceResult()) {
                            if (monster.getVenomMulti() < 3) {
                                monster.setVenomMulti(
                                    (monster.getVenomMulti() + 1)
                                );
                                MonsterStatusEffect monsterStatusEffect = new MonsterStatusEffect(
                                    Collections.singletonMap(
                    ...
                    p.getSkillLevel(
                        PlayerSkillFactory.getSkill(Shadower.VenomousStab)
                    ) >
                    0
                ) {
                    MapleStatEffect venomEffect = PlayerSkillFactory
                        .getSkill(Shadower.VenomousStab)
                        .getEffect(
                            p.getSkillLevel(
                                PlayerSkillFactory.getSkill(
                                    Shadower.VenomousStab
                                )
                            )
                        );
                    for (int i = 0; i < attackCount; i++) {
        ...
        if (totDamage > 1) {
            p
                ...
                .setAttacksWithoutHit(
                    p.getCheatTracker().getAttacksWithoutHit() + 1
                );
            ...
            if (p.getCheatTracker().getAttacksWithoutHit() > offenseLimit) {
                p
                    .getCheatTracker()
                    .registerOffense(
                        CheatingOffense.ATTACK_WITHOUT_GETTING_HIT,
                        Integer.toString(
                            p.getCheatTracker().getAttacksWithoutHit()
                        )
                    );
            }
        }
    ...
    public static void checkHighDamage(
        Player p,
        ...
        if (!p.isGameMaster()) {
            ...
            if (p.getBuffedValue(BuffStat.WK_CHARGE) != null) {
                ...
                PlayerSkill chargeSkill = PlayerSkillFactory.getSkill(
                    chargeSkillId
                );
                maximumDamageToMonster *=
                    chargeSkill
                        .getEffect(p.getSkillLevel(chargeSkill))
                        .getDamage() /
                    100.0;
            }
            if (element != Element.NEUTRAL) {
                ...
                if (
                    attack.skill == Sniper.Blizzard || attack.skill == 3111003
                ) {
                ...
                switch (monster.getEffectiveness(element)) {
                    ...
                    case WEAK:
                        elementalMaxDamagePerMonster =
                            (int) (
                                maximumDamageToMonster * (1.0 + elementalEffect)
                            );
                        break;
                    case STRONG:
                        elementalMaxDamagePerMonster =
            ...
            if (damageToMonster > elementalMaxDamagePerMonster * multiplyer) {
                ...
                if (
                    ...
                    CheatingOffense.DAMAGE_HACK.cheatingSuspicious(
                        ...
                        damageToMonster +
                        " MobID: " +
                        (monster != null ? monster.getId() : "null") +
                        " MobLevel: " +
                        (monster != null ? monster.getLevel() : "null") +
                        "  Map: " +
                        p.getMap().getMapName() +
                        " (" +
                        p.getMapId() +
    ...
    public static final AttackInfo parseDamage(
        PacketReader packet,
        Player p,
        boolean ranged,
        boolean magic
        ...
        if (ranged) {
            ...
            if (
                ret.skill == 0 &&
                p.getJob().getId() != PlayerJob.CORSAIR.getId() &&
                p.getJob().getId() != PlayerJob.BOWMASTER.getId()
            ) {
                if (
                    (ret.attackTime - p.getLastAttackTime() < 110) &&
                    (ret.attackTime - p.getLastAttackTime() >= 0)
                ) {
                    long math = ret.attackTime - p.getLastAttackTime();
                    CheatingOffense.FASTATTACK.cheatingSuspicious(
                        p,
                        "Attacked with a speed of " +
                        math +
                        " and it doesn't seem right.\nMapID of character was " +
                        p.getMapId() +
                        ".\n"
                    );
                } else if (ret.attackTime - p.getLastAttackTime() < 0) {
                    p.dropMessage(
                        "You seem to have a negative attack time... that's not normal! You'll need to post on forums about this and possibly check your Antivirus."
                    );
                }
        ...
        if (magic && ret.skill != 0) {
            calcDmgMax =
                (
                    p.getStat().getTotalMagic() *
                    p.getStat().getTotalMagic() /
                    1000 +
                    p.getStat().getTotalMagic()
                ) /
                30 +
                p.getStat().getTotalInt() /
                200;
        } else if (ret.skill == 4001344) {
            calcDmgMax =
                (p.getStat().getTotalLuk() * 5) *
                p.getStat().getTotalWatk() /
                100;
        } else if (ret.skill == DragonKnight.DragonRoar) {
            calcDmgMax =
                (p.getStat().getTotalStr() * 4 + p.getStat().getTotalDex()) *
                p.getStat().getTotalWatk() /
                100;
            ...
            calcDmgMax =
                (int) (
                    18.5 *
                    (p.getStat().getTotalStr() + p.getStat().getTotalLuk()) +
                    p.getStat().getTotalDex() *
                    2
                ) /
                100 *
                p.getStat().calculateMaxBaseDamage(p.getStat().getTotalWatk());
        } else {
            calcDmgMax =
                p.getStat().calculateMaxBaseDamage(p.getStat().getTotalWatk());
        }
        ...
        if (
            p.getJob().isA((PlayerJob.BOWMAN)) ||
            p.getJob().isA(PlayerJob.THIEF) ||
            p.getJob() == PlayerJob.MARAUDER ||
            p.getJob() == PlayerJob.BUCCANEER
        ) {
            canCrit = true;
        }
        ...
        if (ret.skill != 0) {
            int fixed = ret
                .getAttackEffect(p, PlayerSkillFactory.getSkill(ret.skill))
                .getFixDamage();
            if (fixed > 0) {
                calcDmgMax = fixed;
            }
        ...
        for (int i = 0; i < ret.numAttacked; i++) {
                    ...
                    switch (sourceID) {
                        ...
                        case WhiteKnight.SwordLitCharge:
                            if (
                                monster
                                    .getStats()
                                    .getEffectiveness(Element.LIGHTING) ==
                                ElementalEffectiveness.WEAK
                            ) {
                                calcDmgMax *= 1.05 + level * 0.015;
                            }
            ...
            if (ret.skill != 0) {
                ...
                if (
                    skill.getElement() != Element.NEUTRAL &&
                    p.getBuffedValue(BuffStat.ELEMENTAL_RESET) == null
                ) {
                    if (monster != null) {
                        ElementalEffectiveness eff = monster.getEffectiveness(
                            skill.getElement()
                        );
                ...
                if (ret.skill == Hermit.ShadowWeb) {
                    if (monster != null) {
                        calcDmgMax =
                            monster.getHp() / (50 - p.getSkillLevel(skill));
                    }
                }
            }
            for (int j = 0; j < ret.numDamage; j++) {
                ...
                if (
                    ret.skill == Marksman.Snipe ||
                    (canCrit && damage > hitDmgMax)
                ) {
    ...
    public static void checkDamage(Player p, int damage, AttackInfo ret) {
        if (
            (
                ...
                p
                    .getStat()
                    .calculateWorkingDamageTotal(p.getStat().getTotalWatk()) *
                1.3
            ) &&
            ret.skill == 0
        ) {
            switch (p.getJob().getId()) {
                ...
                case 521:
                    CheatingOffense.DAMAGE_HACK.cheatingSuspicious(
                        ...
                        p
                            .getStat()
                            .calculateWorkingDamageTotal(
                                p.getStat().getTotalWatk()
                            ) +
                        " using a basic attack. Player was level " +
                        p.getLevel() +
                        ". With the JobID " +
                        p.getJob().getId() +
                        "\n"
    ...
    public static AttackInfo parseMesoExplosion(
        PacketReader r,
        AttackInfo ret
    ) {
