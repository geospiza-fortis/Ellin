    ...
    public static void registerClient(final Client c) {
        if (System.currentTimeMillis() - lastUpdate > 600000) {
            ...
            if (load == null || load.size() <= 0) {
                ...
                c
                    .getSession()
                    .write(
                        LoginPackets.GetLoginStatus(
                            CharLoginHeaders.LOGIN_ALREADY
                        )
                    );
                return;
            }
            final double loadFactor =
                1200 / ((double) LoginServer.getUserLimit() / load.size());
        ...
        if (c.finishLogin() == 0) {
            ...
            c
                .getSession()
                .write(
                    LoginPackets.GetLoginStatus(CharLoginHeaders.LOGIN_ALREADY)
                );
        }
    }
}
