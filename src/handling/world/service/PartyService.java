...
public class PartyService {
    ...
    static {
        try (
            PreparedStatement ps = DatabaseConnection
                .getConnection()
                .prepareStatement("UPDATE `characters` SET `party` = -1")
        ) {
            ps.executeUpdate();
            ...
            .forEach(
                partychar -> {
                    ...
                    if (ch > 0) {
                        ...
                        if (
                            p != null && !p.getName().equalsIgnoreCase(namefrom)
                        ) {
                            p
                                .getClient()
                                .write(
    ...
    public static void partyPacket(
        int partyid,
        OutPacket packet,
        MaplePartyCharacter exception
    ) {
        MapleParty party = getParty(partyid);
        if (party == null) {
            return;
            ...
            .forEach(
                partychar -> {
                    ...
                    if (
                        ...
                        (
                            exception == null ||
                            partychar.getId() != exception.getId()
                        )
                    ) {
    ...
    public static void updateParty(
        ...
        switch (operation) {
            ...
            default:
                throw new RuntimeException(
                    "Unhandeled updateParty operation " + operation.name()
                );
        }

        if (
            operation == MaplePartyOperation.LEAVE ||
            operation == MaplePartyOperation.EXPEL
        ) {
        ...
        party
            .getMembers()
            .stream()
            .filter(partyChar -> !(partyChar == null))
            .forEachOrdered(
                partyChar -> {
                    int ch = FindService.findChannel(partyChar.getName());
                    if (ch > 0) {
