...
public class BuddyService {
    ...
    public static MapleBuddyList.BuddyAddResult requestBuddyAdd(
        String addName,
        Player inviter
    ) {
        int ch = FindService.findChannel(addName);
        if (ch > 0) {
            final Player addChar = ChannelServer
                .getInstance(ch)
                .getPlayerStorage()
            ...
            if (addChar != null) {
                ...
                addChar
                    .getClient()
                    .write(
                        PacketCreator.RequestBuddylistAdd(
                            inviter.getId(),
                            inviter.getName()
                        )
                    );
    ...
    public static Pair<MapleBuddyList.BuddyAddResult, String> acceptToInvite(
        Player chr,
        int inviterCid
    ) {
        Iterator<MapleBuddyInvitedEntry> itr = buddyInvited.iterator();
        while (itr.hasNext()) {
            ...
            if (
                inviterCid == inv.inviter &&
                chr.getName().equalsIgnoreCase(inv.name)
            ) {
                itr.remove();
                if (chr.getBuddylist().isFull()) {
                    return new Pair<>(
                        MapleBuddyList.BuddyAddResult.BUDDYLIST_FULL,
                        null
                    );
                }
                ...
                if (ch > 0) {
                    final Player addChar = ChannelServer
                        .getInstance(ch)
                        .getPlayerStorage()
                        .getCharacterById(inviterCid);
                    if (addChar == null) {
                        return new Pair<>(
                            MapleBuddyList.BuddyAddResult.NOT_FOUND,
                            null
                        );
                    }
                    addChar
                    ...
                    addChar
                        .getClient()
                        .write(
                            PacketCreator.UpdateBuddylist(
                                ChannelHeaders.BuddyListHeaders.ADD,
                                addChar.getBuddylist().getBuddies()
                            )
                        );
                    ...
                    chr
                        ...
                        .write(
                            PacketCreator.UpdateBuddylist(
                                ChannelHeaders.BuddyListHeaders.ADD,
                                chr.getBuddylist().getBuddies()
                            )
                        );

                    return new Pair<>(
                        MapleBuddyList.BuddyAddResult.OK,
                        addChar.getName()
                    );
                }
            }
    ...
    public static String denyToInvite(Player p, int inviterCid) {
        ...
        while (itr.hasNext()) {
            ...
            if (
                inviterCid == inv.inviter &&
                p.getName().equalsIgnoreCase(inv.name)
            ) {
                itr.remove();
                final int ch = FindService.findChannel(inviterCid);
                if (ch > 0) {
                    ...
                    addChar.dropMessage(
                        5,
                        p.getName() + " have denied request to be your buddy."
                    );
                    return (
                        "You have denied the buddy request from '" +
                        addChar.getName() +
                        "'"
                    );
                }
    ...
    public static MapleBuddyList.BuddyDelResult DeleteBuddy(
        Player p,
        int deleteCid
    ) {
        ...
        if (ch > 0) {
            final Player delChar = ChannelServer
                .getInstance(ch)
                .getPlayerStorage()
                .getCharacterById(deleteCid);
            if (delChar == null) {
                ...
                if (ch_ <= 0) {
                    final byte result = deleteOfflineBuddy(
                        deleteCid,
                        p.getId()
                    );
                    if (result == -1) {
                    ...
                    p
                        ...
                        .write(
                            PacketCreator.UpdateBuddylist(
                                ChannelHeaders.BuddyListHeaders.REMOVE,
                                p.getBuddylist().getBuddies()
                            )
                        );
            ...
            if (delChar != null) {
                ...
                delChar
                    ...
                    .write(
                        PacketCreator.UpdateBuddylist(
                            ChannelHeaders.BuddyListHeaders.REMOVE,
                            delChar.getBuddylist().getBuddies()
                        )
                    );
                delChar.dropMessage(
                    5,
                    "Your buddy relationship with '" +
                    p.getName() +
                    "' has ended."
                );
    ...
    public static byte deleteOfflineBuddy(final int delId, final int myId) {
        try {
            PreparedStatement ps = DatabaseConnection
                .getConnection()
                .prepareStatement(
                    "DELETE from `buddyentries` WHERE `owner` = ? AND `buddyid` = ?"
                );
            ps.setInt(1, delId);
            ps.setInt(2, myId);
            ps.executeUpdate();
            ...
            if (ch > 0) {
                ...
                if (p != null) {
                    p
                        .getClient()
                        .write(
                            PacketCreator.PrivateChatMessage(
                                nameFrom,
                                chattext,
                                0
                            )
    ...
    private static void updateBuddies(
        int characterId,
        int channel,
        int[] buddies,
        boolean offline,
            ...
            if (ch > 0) {
                ...
                if (p != null) {
                    ...
                    if (ble != null) {
                        ...
                        if (
                            offline ||
                            (isHidden && p.getAdministrativeLevel() < gmLevel)
                        ) {
                            ble.setChannel(-1);
                            mcChannel = -1;
                        } else {
                        ...
                        p
                            .getClient()
                            .write(
                                PacketCreator.UpdateBuddyChannel(
                                    ble.getCharacterId(),
                                    mcChannel
                                )
                            );
