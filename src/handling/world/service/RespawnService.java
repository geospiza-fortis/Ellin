...
public class RespawnService {
    ...
    public static class Respawn implements Runnable {
        ...
        private final List<ChannelServer> cservs = new ArrayList<>(
            World.channelPerThread
        );

        public Respawn(Integer[] chs, int c) {
            StringBuilder s = new StringBuilder(
                "[Respawn Worker] Registering channels "
            );
            for (
                int i = 1;
                (i <= World.channelPerThread) && (chs.length >= c + i);
                i++
            ) {
    ...
    public static void handleMap(
        ...
        final int size,
        final long now,
        ArrayList<Player> chrs,
        ArrayList<MapleMonster> monsters,
        ArrayList<ItemPet> pets
    ) {
        if (map.itemCount() > 0) {
    ...
    public static void handleCooldowns(
        ...
        if (numTimes % 13 == 0) {
            for (ItemPet pet : p.getSummonedPets()) {
                ...
                int newFullness =
                    pet.getFullness() -
                    ItemPetFactory.getHunger(pet.getPetItemId());
                if (newFullness <= 5) {
                    pet.setFullness(15);
                    p.unequipPet(pet, true, true);
                } else {
        ...
        if (hurt && p.isAlive()) {
            if (
                p
                    ...
                    .findById(p.getMap().getHPDecProtect()) ==
                null
            ) {
