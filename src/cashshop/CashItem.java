...
public class CashItem {
    ...
    public Item toItem(CashItem cItem) {
        return toItem(
            cItem,
            InventoryManipulator.getUniqueId(cItem.getItemId(), null),
            0,
            ""
        );
    }
    ...
    public Item toItem(
        ...
        Item ret;
        if (
            ItemConstants.getInventoryType(cItem.getItemId()) ==
            InventoryType.EQUIP
        ) {
            Equip eq = (Equip) ItemInformationProvider
                .getInstance()
                .getEquipById(cItem.getItemId());
            eq.setUniqueId(uniqueId);
            eq.setGiftFrom(gift);
            eq.setExpiration(
                (long) (
                    System.currentTimeMillis() +
                    (long) (nPeriod * 24 * 60 * 60 * 1000)
                )
            );
