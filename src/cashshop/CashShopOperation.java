...
public class CashShopOperation {
    ...
    private static void updateInformation(
        ...
        if (showBuy) {
            c
                .getSession()
                .write(
                    CashShopPackets.ShowBoughtCashItem(item, c.getAccountID())
                );
        }
    ...
    private static void BuyCashItem(Player p, PacketReader packet) {
        ...
        if (
            cashItem == null ||
            !cashItem.isOnSale() ||
            p.getCashShop().getCash(currencyType) < cashItem.getPrice()
        ) {
            p
                .getClient()
                .getSession()
                .write(
                    CashShopPackets.BuyError(
                        CashShopPackets.ERROR_INSUFFICIENT_CASH
                    )
                );
            return;
        }
        if (p.getCashShop().isFull()) {
            p
                ...
                .write(
                    CashShopPackets.BuyError(
                        CashShopPackets.ERROR_INVENTORY_FULL
                    )
                );
            return;
        }
        if (!cashItem.genderEquals(p.getGender())) {
            p
                ...
                .write(
                    CashShopPackets.BuyError(
                        CashShopPackets.ERROR_GENDER_RESTRICTIONS
                    )
                );
            return;
        }
    ...
    private static void GiftItem(Player p, PacketReader packet) {
        ...
        if (!CashShopTools.checkBirthday(p.getClient(), birthday)) {
            p
                ...
                .getSession()
                .write(
                    CashShopPackets.GiftError(CashShopPackets.ERROR_BIRTHDAY)
                );
            return;
        }
        ...
        if (item == null || !item.isOnSale()) {
            p
                ...
                .getSession()
                .write(
                    CashShopPackets.GiftError(
                        CashShopPackets.ERROR_OUT_OF_STOCK
                    )
                );
            return;
        }
        if (
            p.getCashShop().getCash(1) < item.getPrice() ||
            message.length() > 78 ||
            message.length() < 1 ||
            item.getPrice() <= 0
        ) {
        ...
        if (recipientAcct == -1) {
            p
                ...
                .write(
                    CashShopPackets.GiftError(
                        CashShopPackets.ERROR_RECIPIENT_NAME
                    )
                );
            return;
        }
        if (!item.genderEquals(PlayerQuery.getGenderById(recipientAcct))) {
            p
                ...
                .write(
                    CashShopPackets.GiftError(
                        CashShopPackets.ERROR_GIFT_ITEM_RECEIVER_GENDER
                    )
                );
            return;
        }
    ...
    private static void BuyInventorySlots(Player p, PacketReader packet) {
        ...
        if (
            currentSlots + 4 > getMaxInventorySlots(p.getJob().getId(), invType)
        ) {
            p
                .getClient()
                .getSession()
                .write(
                    CashShopPackets.BuyInventorySlotsError(
                        CashShopPackets.ERROR_TOO_MANY_CASH_ITEMS
                    )
                );
            return;
        }
        ...
        if (p.gainSlots(invType.getType(), 4, false)) {
            p
                ...
                .getSession()
                .write(
                    CashShopPackets.UpdateInventorySlots(
                        invType.getType(),
                        p.getSlots(invType.getType())
                    )
                );
            updateInformation(p.getClient(), null, false);
        }
    ...
    private static void CashInventorySlots(Player p, PacketReader packet) {
        ...
        if (
            currentSlots +
            4 >
            CashShopTools.getMaxInventorySlots(
                p.getJob().getId(),
                InventoryType.CASH
            )
        ) {
            p
                ...
                .getSession()
                .write(
                    CashShopPackets.BuyStorageSlotsError(
                        CashShopPackets.ERROR_TOO_MANY_CASH_ITEMS
                    )
                );
            return;
        ...
        p
            ...
            .getSession()
            .write(
                CashShopPackets.UpdateStorageSlots(
                    (short) p.getStorage().getSlots()
                )
            );
        updateInformation(p.getClient(), null, false);
    }
    ...
    private static void BuyCharacterSlots(Player p, PacketReader packet) {
        ...
        if (currentSlots + 1 > 6) {
            p
                ...
                .getSession()
                .write(
                    CashShopPackets.BuyCharacterSlotsError(
                        CashShopPackets.ERROR_TOO_MANY_CASH_ITEMS
                    )
                );
            return;
        ...
        if (p.getClient().gainCharacterSlot()) {
            ...
            p
                ...
                .getSession()
                .write(
                    CashShopPackets.UpdateCharacterSlots(
                        p.getClient().getCharacterSlots()
                    )
                );
            updateInformation(p.getClient(), null, false);
        } else {
    ...
    private static void TakeFromCashInventory(Player p, PacketReader packet) {
        ...
        if (cashItem == null) {
            CheatingOffense.PACKET_EDIT.cheatingSuspicious(
                p,
                "Tried to transfer non-existent cashshop item from staging"
            );
            p
                .getClient()
                .getSession()
                .write(
                    CashShopPackets.TakeError(CashShopPackets.ERROR_UNKNOWN)
                );
            return;
        }
        if (
            cashItem.getQuantity() > 0 &&
            !InventoryManipulator.checkSpace(
                p.getClient(),
                cashItem.getItemId(),
                cashItem.getQuantity(),
                cashItem.getOwner()
            )
        ) {
            p
        ...
        short position = InventoryManipulator.addbyItem(
            p.getClient(),
            item,
            true
        );
        ...
        p
            .getClient()
            .getSession()
            .write(CashShopPackets.TakeFromCashInventory(cashItem, position));
    }
    ...
    private static void PutIntoCashInventory(Player p, PacketReader packet) {
        ...
        if (item == null) {
            p
                ...
                .getSession()
                .write(
                    CashShopPackets.PlaceError(CashShopPackets.ERROR_UNKNOWN)
                );
            return;
        }
        ...
        p
            .getClient()
            .announce(
                CashShopPackets.PutIntoCashInventory(
                    item,
                    p.getClient().getAccountID()
                )
            );
        p
            .getClient()
            .announce(CashShopPackets.ShowCashInventory(p.getClient()));
    }
    ...
    private static void BuyQuestItem(Player p, PacketReader packet) {
        ...
        if (serialNumber / 10000000 != 8) {
            p
                ...
                .getSession()
                .write(
                    CashShopPackets.BuyError(CashShopPackets.ERROR_OUT_OF_STOCK)
                );
            return;
        }
        if (item == null || !item.isOnSale()) {
        ...
        if (p.getMeso() >= item.getPrice()) {
            if (
                ...
                p
                    ...
                    .getSession()
                    .write(
                        CashShopPackets.BuyError(CashShopPackets.ERROR_UNKNOWN)
                    );
            }
        } else {
            p
                ...
                .write(
                    CashShopPackets.BuyError(
                        CashShopPackets.ERROR_INSUFFICIENT_MESOS
                    )
                );
        }
    }
    ...
    private static void BuyPackage(Player p, PacketReader packet) {
        ...
        List<Item> serialNumbers = CashItemFactory.getPackage(
            cashItem.getItemId()
        );
        ...
        Item item = cashItem.toItem(cashItem);
        List<Item> cashPackage = CashItemFactory.getPackage(
            cashItem.getItemId()
        );
        ...
        p
            .getClient()
            .getSession()
            .write(CashShopPackets.ShowCashInventory(p.getClient()));
    }
    ...
    private static void GiftPackage(Player p, PacketReader packet) {
        ...
        if (cashItem == null || !cashItem.isOnSale()) {
            CheatingOffense.PACKET_EDIT.cheatingSuspicious(
                p,
                "Tried to buy nonexistent package from cash shop"
            );
            p
                .getClient()
                .getSession()
        ...
        if (p.getCashShop().getCash(4) < cashItem.getPrice()) {
            CheatingOffense.PACKET_EDIT.cheatingSuspicious(
                p,
                "Tried to gift item from cash shop with nonexistent cash"
            );
            p
                .getClient()
                .getSession()
    ...
    private static void BuyFriendshipAndCoupleRing(
        Player p,
        PacketReader r,
        boolean couple
    ) {
        ...
        if (p.getCashShop().getCash(currencyType) < ring.getPrice()) {
            CheatingOffense.PACKET_EDIT.cheatingSuspicious(
                p,
                "Tried to buy friendship ring from cash shop with nonexistent cash"
            );
            p
                .getClient()
                .getSession()
        ...
        if (
            p.getGender() == gender &&
            GameConstants.GENDER_RESTRICT_RINGS &&
            couple
        ) {
            p
                .getClient()
        ...
        boolean creationRing = ItemRing.createRing(
            ring.getItemId(),
            p,
            recipientAcct,
            message,
        ...
        PlayerNote.sendNote(
            p,
            PlayerQuery.getNameById(recipientAcct),
            message,
            0
        );
        Player partnerChar = p
            .getClient()
            .getChannelServer()
            .getPlayerStorage()
