...
public class CashCouponFactory {
        ...
        try {
            try (
                PreparedStatement ps = DatabaseConnection
                    .getConnection()
                    .prepareStatement(
                        "SELECT `used` FROM `cashshopcouponitems` WHERE `code` = ?"
                    )
            ) {
                ps.setString(1, code);
        ...
        try {
            try (
                PreparedStatement ps = DatabaseConnection
                    .getConnection()
                    .prepareStatement(
                        "SELECT `type`, `itemData`, `quantity` FROM `cashshopcouponitems` WHERE `code` = ?"
                    )
            ) {
                ps.setString(1, code);
                try (ResultSet rs = ps.executeQuery()) {
        ...
        try {
            try (
                PreparedStatement ps = DatabaseConnection
                    .getConnection()
                    .prepareStatement(
                        "UPDATE `cashshopcouponitems` SET `character` = ?, `used` = 1 WHERE code = ?"
                    )
            ) {
                ps.setString(1, name);
                ps.setString(2, code);
        ...
        try {
            try (
                PreparedStatement ps = DatabaseConnection
                    .getConnection()
                    .prepareStatement(
                        "DELETE from `cashshopcouponitems` WHERE `code` = ?"
                    )
            ) {
                ps.setString(1, code);
                ps.execute();
    ...
    public static Pair<Pair<Integer, Integer>, Pair<List<Item>, Integer>> getSize(
        ...
        for (CashCouponData hmm : ccd) {
            switch (hmm.getType()) {
                ...
                case 2:
                    if (
                        hmm.getQuantity() <= Short.MAX_VALUE &&
                        hmm.getQuantity() > 0
                    ) {
                        togiveII.add(
                            new Item(
        ...
        return new Pair<>(
            new Pair<>(MaplePoints, Cashsize),
            new Pair<>(togiveII, mesos)
        );
    }
}
