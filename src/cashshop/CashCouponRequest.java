    ...
    public static void CouponCode(PacketReader packet, Client c) {
        ...
        if (code == null || code.length() < 16 || code.length() > 32) {
            c
                .getSession()
                .write(
                    CashShopPackets.CouponError(
                        CashShopPackets.ERROR_COUPON_NUMBER
                    )
                );
            return;
        }

        final boolean validCode = CashCouponFactory.getCouponCodeValid(
            code.toUpperCase()
        );
        if (!validCode) {
            c
        ...
        final List<CashCouponData> rewards = CashCouponFactory.getCcData(
            code.toUpperCase()
        );
        if (rewards == null) {
        ...
        final Pair<Pair<Integer, Integer>, Pair<List<Item>, Integer>> cscsize = CashCouponFactory.getSize(
            rewards
        );
        if (
            (
                c.getPlayer().getCashShop().getCash(2) +
                cscsize.getLeft().getLeft()
            ) <
            0
        ) {
            return;
        ...
        if (
            c.getPlayer().getCashShop().getItemsSize() >=
            (100 - cscsize.getLeft().getRight())
        ) {
            c
                .getSession()
                .write(
        ...
        if (
            !ItemConstants.haveSpace(
                c.getPlayer(),
                cscsize.getRight().getLeft()
            )
        ) {
            c
                .getSession()
        ...
        for (final CashCouponData reward : rewards) {
            switch (reward.getType()) {
                ...
                case 1:
                    {
                        final CashItem item = CashItemFactory.getItem(
                            reward.getData()
                        );
                        if (item != null) {
                ...
                case 2:
                    {
                        if (
                            reward.getQuantity() <= Short.MAX_VALUE &&
                            reward.getQuantity() > 0
                        ) {
                            final boolean pos = InventoryManipulator.addById(
                                c,
                                reward.getData(),
