...
public class NPCConversationManager extends AbstractPlayerInteraction {
    ...
    public NPCConversationManager(
        Client c,
        int npc,
        List<MaplePartyCharacter> otherParty,
        MCParty mParty
    ) {
        super(c);
        this.c = c;
    ...
    public void sendNext(String text) {
        getClient()
            .getSession()
            .write(PacketCreator.GetNPCTalk(npc, (byte) 0, text, "00 01"));
    }

    public void sendPrev(String text) {
        getClient()
            .getSession()
            .write(PacketCreator.GetNPCTalk(npc, (byte) 0, text, "01 00"));
    }

    public void sendNextPrev(String text) {
        getClient()
            .getSession()
            .write(PacketCreator.GetNPCTalk(npc, (byte) 0, text, "01 01"));
    }

    public void sendOk(String text) {
        getClient()
            .getSession()
            .write(PacketCreator.GetNPCTalk(npc, (byte) 0, text, "00 00"));
    }

    public void sendYesNo(String text) {
        getClient()
            .getSession()
            .write(PacketCreator.GetNPCTalk(npc, (byte) 1, text, ""));
    }

    public void sendAcceptDecline(String text) {
        getClient()
            .getSession()
            .write(PacketCreator.GetNPCTalk(npc, (byte) 0x0C, text, ""));
    }

    public void sendSimple(String text) {
        getClient()
            .getSession()
            .write(PacketCreator.GetNPCTalk(npc, (byte) 4, text, ""));
    }
    ...
    public void sendStyle(String text, int styles[]) {
        getClient()
            .getSession()
            .write(PacketCreator.GetNPCTalkStyle(npc, text, styles));
    }

    public void sendGetNumber(String text, int def, int min, int max) {
        getClient()
            .getSession()
            .write(PacketCreator.GetNPCTalkNum(npc, text, def, min, max));
    }
    ...
    public void gainExp(int gain) {
        getPlayer()
            .gainExperience(
                gain * c.getChannelServer().getExpRate(),
                true,
                true
            );
    }
    ...
    public void teachSkill(int id, int level, int masterlevel) {
        getPlayer()
            .changeSkillLevel(
                PlayerSkillFactory.getSkill(id),
                level,
                masterlevel
            );
    }
    ...
    public void rechargeStars() {
        ...
        Item stars = getPlayer()
            .getInventory(InventoryType.USE)
            .getItem((byte) 1);
        if (
            ItemConstants.isThrowingStar(stars.getItemId()) ||
            ItemConstants.isBullet(stars.getItemId())
        ) {
    ...
    public void playSound(String sound) {
        getClient()
            .getPlayer()
            .getMap()
            .broadcastMessage(EffectPackets.PlaySound(sound));
    }
            ...
            .map(
                chr ->
                    c
                        .getChannelServer()
                        .getPlayerStorage()
                        .getCharacterByName(chr.getName())
            )
            .filter(
                curChar ->
                    (
    ...
    public String startSpeedQuiz() {
        ...
        if (time > 0) {
            ...
            if (!can) {
                ...
                return (
                    "You've already tried the speed quiz in the past hour. Please come back again in " +
                    remaining +
                    " minutes."
                );
            }
    ...
    public void warpAllInMap(int mapid, int portal) {
        ...
        for (Player p : outMap.getCharacters()) {
            ...
            p
                .getClient()
                .getPlayer()
                .changeMap(outMap, outMap.getPortal(portal));
            outMap = mapFactory.getMap(mapid);
            p
                .getClient()
                .getPlayer()
                .getEventInstance()
                .unregisterPlayer(p.getClient().getPlayer());
        }
    }
    ...
    public void warpRandom(int mapid) {
        ...
        Portal portal = target.getPortal(
            rand.nextInt(target.getPortals().size())
        );
    ...
    public void challengeParty(MCParty party, int field) {
        ...
        if (leader != null && leader.getMCPQField() != null) {
            if (!leader.isChallenged()) {
                ...
                for (MaplePartyCharacter fucker : c
                    .getPlayer()
                    .getParty()
                    .getMembers()) {
                    members.add(fucker);
                ...
                sendOk(
                    "The other party is responding to a different challenge."
                );
            }
        } else {
    ...
    public void addRandomItem(int id) {
        ...
        InventoryManipulator.addFromDrop(
            getClient(),
            i.randomizeStats((Equip) i.getEquipById(id)),
            "",
            true
        );
    }
    ...
    public void GachaMessage(int itemid, boolean rare) throws Exception {
        ...
        String mapName = getClient()
            .getPlayer()
            .getMapName(
                gacMap[(getNpc() != 9100117 && getNpc() != 9100109)
                        ? (getNpc() - 9100100)
                        : getNpc() == 9100109 ? 8 : 9]
            );
        if (!rare) {
    ...
    public int countReactor() {
        Field map = c.getPlayer().getMap();
        double range = Double.POSITIVE_INFINITY;
        List<FieldObject> reactors = map.getMapObjectsInRange(
            c.getPlayer().getPosition(),
            range,
    ...
    public void reloadChar() {
        getPlayer()
            .getClient()
            .getSession()
            .write(PacketCreator.GetCharInfo(getPlayer()));
    ...
    public short gainItemRetPos(int itemid) {
        ...
        c
            .getSession()
            .write(PacketCreator.AddInventorySlot(InventoryType.EQUIP, it));
        c
            .getSession()
            .write(PacketCreator.GetShowItemGain(itemid, (short) 1, true));
    ...
    public void showInventory(int type) {
        ...
        Inventory invy = c
            .getPlayer()
            .getInventory(InventoryType.getByType((byte) type));
        send =
            invy
                .list()
                .stream()
    ...
    public boolean isUsingOldPqNpcStyle() {
        return (
            GameConstants.USE_OLD_GMS_STYLED_PQ_NPCS &&
    ...
    public int partyMembersInMap() {
        ...
        inMap =
            getPlayer()
                ...
                .getCharacters()
                .stream()
                .filter(char2 -> (char2.getParty() == getPlayer().getParty()))
                .map(_item -> 1)
                .reduce(inMap, Integer::sum);
        return inMap;
    }
    ...
    public void showFredrick() {
        getClient()
            .getSession()
            .write(InteractionPackets.GetFredrick(getPlayer(), 0x23));
    }
        ...
        try {
            if (
                !ItemFactory.MERCHANT
                    .loadItems(getPlayer().getId(), false)
                    .isEmpty()
            ) {
                return true;
            }
    ...
    public void resetStats() {
        int totAp =
            getPlayer().getStat().getStr() +
            getPlayer().getStat().getDex() +
            getPlayer().getStat().getLuk() +
            getPlayer().getStat().getInt() +
            getPlayer().getStat().getRemainingAp();
    ...
    public final void sendRPS() {
        c
            .getSession()
            .write(
                PacketCreator.GetRockPaperScissorsMode((byte) 8, -1, -1, -1)
            );
    }
    ...
    public void displayDrops(int selection) {
        ...
        String text = "";
        List ranks = MapleMonsterInformationProvider
            .getInstance()
            .retrieveDrop(job.getId());
                    ...
                    if (
                        ...
                        if (num == 0) {
                            text =
                                new StringBuilder()
                                    .append(text)
                                    .append("Drops monster #e")
                                    .append(job.getStats().getName())
                                    .append("#n:\r\n")
                                    .toString();
                            text =
                                new StringBuilder()
                        ...
                        percent =
                            Integer
                                .valueOf(
                                    de.chance == 999999 ? 1000000 : de.chance
                                )
                                .doubleValue() /
                            10000.0D *
                            ChannelServer
    ...
    public void searchToItemId(String message) {
        ...
        try {
            ...
            String nameItem = ItemInformationProvider
                .getInstance()
                .getName(itemid);
            ...
            PreparedStatement ps = con.prepareStatement(
                "SELECT dropperid FROM drop_data WHERE itemid = ?"
            );
            ps.setInt(1, itemid);
            ...
            while (rs.next()) {
                ...
                for (MapleData mobIdData : data.getChildren()) {
                    ...
                    String mobNameFromData = MapleDataTool.getString(
                ...
                for (Pair<Integer, String> mobPair : mobPairList) {
                    if (
                        mobPair.getLeft() == (mobn) &&
                        !retMobs.contains(mobPair.getRight())
                    ) {
    ...
    public void searchToNameItem(String name) {
        ...
        msg =
            new StringBuilder()
                .append(msg)
                .append("Name of found items:\r\n")
                .toString();
        for (Map.Entry<Integer, String> itemEntry : miip
            .getAllItems()
            .entrySet()) {
            if (
                itemEntry.getValue().toLowerCase().contains(name.toLowerCase())
            ) {
    ...
    public void listMonsters() {
        ...
        List<FieldObject> monsters = getPlayer()
            .getMap()
            .getMapObjectsInRange(
                this.c.getPlayer().getPosition(),
                (1.0D / 0.0D),
                Arrays.asList(new FieldObjectType[] { FieldObjectType.MONSTER })
            );
        ...
        for (int i = 0; i < monstersInMap.size(); i++) {
            monster =
                MapleLifeFactory
                    .getMonster(((Integer) monstersInMap.get(i)).intValue())
                    .getName();
            text =
                new StringBuilder()
                    .append(text)
                    .append("#L")
                    .append(monstersInMap.get(i))
                    .append("#")
                    .append(monster)
                    .append("#l\r\n")
                    .toString();
    ...
    private int getMobsIDFromName(String search) {
        ...
        for (Pair<Integer, String> mobPair : mobPairList) {
            if (
                mobPair.getRight().toLowerCase().equals(search.toLowerCase()) &&
                mobPair.getLeft() > 0
            ) {
