...
public class EventManager {
    ...
    private Lock lobbyLock = new MonitoredReentrantLock(
        MonitoredLockType.EM_LOBBY
    );
    private Lock queueLock = new MonitoredReentrantLock(
        MonitoredLockType.EM_QUEUE
    );
    ...
    private List<Integer> getLobbyRange() {
        try {
            return ConvertTool.ConvertFromScriptInt(
                iv.invokeFunction("setLobbyRange", (Object) null)
            );
        } catch (ScriptException | NoSuchMethodException ex) {
        ...
        try {
            ...
            EventInstanceManager eim = (EventInstanceManager) (
                iv.invokeFunction(
                    "setup",
                    difficulty,
                    (lobbyId > -1) ? lobbyId : leader.getId()
                )
            );
            if (eim == null) {
        ...
        try {
            ...
            EventInstanceManager eim = (EventInstanceManager) (
                iv.invokeFunction(
                    "setup",
                    difficulty,
                    (lobbyId > -1) ? lobbyId : party.getLeaderId()
                )
            );
            if (eim == null) {
                if (lobbyId > -1) setLockLobby(lobbyId, false);
    ...
    public List<MaplePartyCharacter> getEligibleParty(MapleParty party) {
        ...
        try {
            Object p = iv.invokeFunction(
                "getEligibleParty",
                party.getPartyMembers()
            );

            if (p != null) {
                List<MaplePartyCharacter> lmpc = ConvertTool.ConvertFromScriptArray(
                    p
                );
    ...
    private void exportReadyGuild(Integer guildId) {
        ...
        String callout =
            "[Guild Quest] Your guild has been registered to attend to the Sharenian Guild Quest at channel " +
            this.getChannelServer().getChannel() +
    ...
    private void exportMovedQueueToGuild(Integer guildId, int place) {
        ...
        String callout =
            ...
            GameConstants.ordinal(place) +
            " place on the waiting queue.";
    ...
    private void instantiateQueuedInstance() {
        ...
        try {
            if (
                readyInstances.size() >= Math.ceil((double) maxLobbys / 3.0)
            ) return;

            readyInstances.add(
                new EventInstanceManager(this, "sampleName" + readyId)
            );
            readyId++;
        } finally {
            queueLock.unlock();
