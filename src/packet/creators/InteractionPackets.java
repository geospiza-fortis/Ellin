...
public class InteractionPackets {
    ...
    public static OutPacket GetHiredMerchant(
        Client c,
        Minigame miniGame,
        String description
    ) {
    ...
    public static OutPacket RetrieveFirstMessage() {
        final WritingPacket wp = new WritingPacket();
        wp.writeShort(
            SendPacketOpcode.SHOW_HIRED_MERCHANT_AGREEMENT.getValue()
        );
        wp.write(0x09);
        return wp.getPacket();
    }
    ...
    public static OutPacket GetPlayerShopChat(
        Player p,
        String chat,
        boolean owner
    ) {
    ...
    public static OutPacket ShopScannerResult(
        Client c,
        boolean displayTopTen,
        int itemid,
        List<Pair<PlayerShopItem, AbstractMapleFieldObject>> hmsAvailable,
        ConcurrentLinkedQueue<Integer> mostSearched
    ) {
