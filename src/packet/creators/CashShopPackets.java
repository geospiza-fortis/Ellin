...
public class CashShopPackets {
    ...
    private static byte INVENTORY = 0x2F, GIFTS = 0x31, DISPLAY_WISH_LIST =
        0x33, UPDATE_WISH_LIST = 0x39, WISH_LIST_ERROR =
        0x3A, INSERT_TO_STAGING = 0x3B, BUY_ERROR = 0x3C, REDEEM_COUPON =
        0x3D, COUPON_ERROR = 0x40, UPDATE_INVENTORY_SLOTS =
        0x44, INVENTORY_SLOTS_ERROR = 0x45, UPDATE_STORAGE_SLOTS =
        0x46, STORAGE_SLOTS_ERROR = 0x47, UPDATE_CHARACTER_SLOTS =
        0x48, CHARACTER_SLOTS_ERROR = 0x49, MOVE_FROM_STAGING =
        0x4A, MOVE_FROM_STAGING_ERROR = 0x4B, MOVE_TO_STAGING =
        0x4C, MOVE_TO_STAGING_ERROR = 0x4D, EXPIRE_ITEM = 0x4E, SEND_GIFT =
        0x6B, GIFT_ERROR = 0x6C, BUY_MESO_ITEM = 0x6D;

    public static byte ERROR_UNKNOWN =
        0x00, ERROR_UNKNOWN_THEN_EXIT_TO_CHANNEL_1 = (byte) 0x7F, ERROR_REQUEST_TIMED_OUT = (byte) 0x80, ERROR_UNKNOWN_WARP_TO_CHANNEL_2 = (byte) 0x81, ERROR_INSUFFICIENT_CASH = (byte) 0x82, ERROR_AGE_LIMIT = (byte) 0x83, ERROR_EXCEEDED_ALLOTTED_LIMIT_OF_PRICE = (byte) 0x84, ERROR_TOO_MANY_CASH_ITEMS = (byte) 0x85, ERROR_GIFT_ITEM_RECEIVER_GENDER = (byte) 0x86, ERROR_COUPON_NUMBER = (byte) 0x87, ERROR_COUPON_EXPIRED = (byte) 0x88, ERROR_COUPON_USED = (byte) 0x89, ERROR_NEXON_CAFE_ONLY_COUPON = (byte) 0x8A, ERROR_USED_NEXON_CAFE_ONLY_COUPON = (byte) 0x8B, ERROR_EXPIRED_NEXON_CAFE_ONLY_COUPON = (byte) 0x8C, ERROR_IS_COUPON_NUMBER = (byte) 0x8D, ERROR_GENDER_RESTRICTIONS = (byte) 0x8E, ERROR_REGULAR_ITEM_ONLY_COUPON = (byte) 0x8F, ERROR_MAPLESTORY_ONLY_COUPON_NO_GIFTS = (byte) 0x90, ERROR_INVENTORY_FULL = (byte) 0x91, ERROR_PREMIUM_SERVICE = (byte) 0x92, ERROR_INVALID_RECIPIENT = (byte) 0x93, ERROR_RECIPIENT_NAME = (byte) 0x94, ERROR_OUT_OF_STOCK = (byte) 0x96, ERROR_INSUFFICIENT_MESOS = (byte) 0x98, ERROR_CASH_SHOP_IN_BETA = (byte) 0x99, ERROR_BIRTHDAY = (byte) 0x9A, ERROR_ONLY_AVAILABLE_FOR_GIFTS = (byte) 0x9D, ERROR_ALREADY_APPLIED = (byte) 0x9E, ERROR_DAILY_PURCHASE_LIMIT = (byte) 0xA3, ERROR_MAXIMUM_USAGE = (byte) 0xA6, ERROR_COUPON_SYSTEM_COMING_SOON = (byte) 0xA7, ERROR_ITEM_ONLY_USABLE_15_DAYS_AFTER_REGGING = (byte) 0xA8, ERROR_INSUFFICIENT_GIFT_TOKENS = (byte) 0xA9, ERROR_SEND_TECHNICAL_DIFFICULTIES = (byte) 0xAA, ERROR_SEND_LESS_THAN_2_WEEKS_SINCE_FIRST_CHARGE = (byte) 0xAB, ERROR_CANNOT_GIVE_WITH_BAN_HISTORY = (byte) 0xAC, ERROR_GIFT_LIMITATION = (byte) 0xAD, ERROR_TOO_LATE_TO_GIVE = (byte) 0xAE, ERROR_GIFT_TECHNICAL_DIFFICULTIES = (byte) 0xAF, ERROR_TRANSFER_TO_WORLD_UNDER_20 = (byte) 0xB0, ERROR_TRANSFER_TO_SAME_WORLD = (byte) 0xB1, ERROR_TRANSFER_TO_YOUNG_WORLD = (byte) 0xB2, ERROR_WORLD_CHARACTER_SLOTS_FULL = (byte) 0xB3, ERROR_EVENT_NOT_AVAILABLE_OR_EXPIRED = (byte) 0xCE;
    ...
    public static void AddCashItemInformation(
    ...
    private static OutPacket ErrorSimple(byte header, byte message) {
        WritingPacket wp = new WritingPacket(
            message != ERROR_OUT_OF_STOCK ? 4 : 8
        );
        wp.writeShort(SendPacketOpcode.CASH_SHOP.getValue());
        wp.write(header);
        wp.write(message);
    ...
    public static OutPacket UpdateInventorySlots(
        byte invType,
        short newCapacity
    ) {
    ...
    public static OutPacket GiftSent(
        String recipient,
        int itemId,
        int price,
        int quantity,
    ...
    public static void AddCashItemInformation(
        ...
        wp.writeAsciiString(
            StringUtil.getRightPaddedStr(item.getGiftFrom(), '\0', 13)
        );
        if (isGift) {
            wp.writeAsciiString(
                StringUtil.getRightPaddedStr(giftMessage, '\0', 73)
            );
            return;
        }
    ...
    public static OutPacket CouponRewards(
        final int accid,
        final int MaplePoints,
        final Map<Integer, Item> items1,
