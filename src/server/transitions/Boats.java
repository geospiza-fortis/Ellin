    ...
    public void invasionBalrog() {
        ...
        if (numberSpawns > 0) {
            for (int i = 0; i < numberSpawns; i++) {
                boatToOrbis.spawnMonsterOnGroudBelow(
                    MapleLifeFactory.getMonster(8150000),
                    new Point(485, -221)
                );
                boatToEllinia.spawnMonsterOnGroudBelow(
                    MapleLifeFactory.getMonster(8150000),
                    new Point(-590, -221)
                );
            }
            ...
            boatToOrbis.broadcastMessage(
                EffectPackets.MusicChange("Bgm04/ArabPirate")
            );
            boatToEllinia.broadcastMessage(
                EffectPackets.MusicChange("Bgm04/ArabPirate")
            );
