...
public class MapleQuest {
    ...
    public boolean canComplete(Player c, Integer npcid) {
    ...
    public void complete(Player c, int npc) {
        complete(c, npc, null);
    }

    public void complete(Player p, int npc, Integer selection) {
        if (autoPreComplete || canComplete(p, npc)) {
            for (MapleQuestAction a : completeActs.values()) {
                if (!a.check(p, selection)) {
                    return;
                }
            }
            forceComplete(p, npc);
    ...
    public boolean forceComplete(Player c, int npc) {
        if (timeLimit > 0) {
            c.announce(PacketCreator.RemoveQuestTimeLimit(id));
        }
        MapleQuestStatus newStatus = new MapleQuestStatus(
