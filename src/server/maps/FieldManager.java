...
public class FieldManager {
    ...
    public FieldManager(
        ...
        ReentrantReadWriteLock rrwl = new MonitoredReentrantReadWriteLock(
            MonitoredLockType.MAP_FACTORY
        );
    ...
    private synchronized Field loadMapFromWz(
        int mapid,
        Integer omapid,
        boolean respawns,
        boolean npcs,
        ...
        for (MapleData footRoot : mapData.getChildByPath("foothold")) {
            for (MapleData footCat : footRoot) {
                for (MapleData footHold : footCat) {
                    int x1 = MapleDataTool.getInt(
                        footHold.getChildByPath("x1")
                    );
                    int y1 = MapleDataTool.getInt(
                        footHold.getChildByPath("y1")
                    );
                    int x2 = MapleDataTool.getInt(
                        footHold.getChildByPath("x2")
                    );
                    int y2 = MapleDataTool.getInt(
                        footHold.getChildByPath("y2")
                    );
                    ...
                    fh.setPrev(
                        MapleDataTool.getInt(footHold.getChildByPath("prev"))
                    );
                    fh.setNext(
                        MapleDataTool.getInt(footHold.getChildByPath("next"))
                    );
            ...
            try (
                ...
                try (ResultSet rs = ps.executeQuery()) {
                    while (rs.next()) {
                        ...
                        AbstractLoadedMapleLife myLife = loadLife(
                            id,
                            f,
                            hide,
                            fh,
        ...
        if (reactors && mapData.getChildByPath("reactor") != null) {
            for (MapleData reactor : mapData.getChildByPath("reactor")) {
                String id = MapleDataTool.getString(
                    reactor.getChildByPath("id")
                );
                if (id != null) {
    ...
    private AbstractLoadedMapleLife loadLife(
        int id,
        int f,
        boolean hide,
        int fh,
    ...
    private AbstractLoadedMapleLife loadLife(
        ...
        myLife.setPosition(
            new Point(
                MapleDataTool.getInt(life.getChildByPath("x")),
                MapleDataTool.getInt(life.getChildByPath("y"))
            )
        );

        if (
            MapleDataTool.getInt("hide", life, 0) == 1 &&
            myLife instanceof MapleNPC
        ) {
            myLife.setHide(true);
        }
        return myLife;
    ...
    private Reactor loadReactor(MapleData reactor, String id) {
        Reactor myReactor = new Reactor(
            ReactorFactory.getReactor(Integer.parseInt(id)),
            Integer.parseInt(id)
        );
        ...
        myReactor.setDelay(
            MapleDataTool.getInt(reactor.getChildByPath("reactorTime")) * 1000
        );
        myReactor.setState((byte) 0);
        myReactor.setName(
            MapleDataTool.getString(reactor.getChildByPath("name"), "")
        );
        return myReactor;
    }

    private String getMapName(int mapid) {
        String mapName = StringUtil.getLeftPaddedStr(
    ...
    private String getMapStringName(int mapid) {
        ...
        if (mapid < 100000000) builder.append("maple"); else if (
            mapid >= 100000000 && mapid < 200000000
        ) builder.append("victoria"); else if (
            mapid >= 200000000 && mapid < 300000000
        ) builder.append("ossyria"); else if (
            mapid >= 540000000 && mapid < 541010110
        ) builder.append("MasteriaGL"); else if (
            mapid >= 600000000 && mapid < 620000000
        ) builder.append("singapore"); else if (
            mapid >= 670000000 && mapid < 682000000
        ) builder.append("weddingGL"); else if (
            mapid >= 682000000 && mapid < 683000000
        ) builder.append("HalloweenGL"); else if (
            mapid >= 800000000 && mapid < 900000000
        ) builder.append("jp"); else builder.append("etc");
