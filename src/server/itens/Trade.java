...
public class Trade {
    ...
    public final void CompleteTrade() {
            ...
            itemz.forEach(
                item -> {
                    InventoryManipulator.addFromDrop(
                        chr.get().getClient(),
                        item,
                        "Completed trade with " +
                        partner.getChr().getName() +
                        ". " +
                        chr.get().getName() +
                        " received the item.",
                        false
        ...
        chr
            ...
            .getClient()
            .getSession()
            .write(InteractionPackets.GetTradeCompletion(tradingslot));
    }
    ...
    public final void cancel(final Player chr, final int unsuccessful) {
        ...
        chr
            .getClient()
            .getSession()
            .write(InteractionPackets.GetTradeCancel(tradingslot));
    }
    ...
    public final void setMeso(final int meso) {
        ...
        if (chr.get().getMeso() >= meso) {
            ...
            chr
                ...
                .getClient()
                .getSession()
                .write(InteractionPackets.GetTradeMesoSet((byte) 0, this.meso));
            if (partner != null) {
                partner
                    .getChr()
                    .getClient()
                    .getSession()
                    .write(
                        InteractionPackets.GetTradeMesoSet((byte) 1, this.meso)
                    );
            }
        }
    }
    ...
    public final void addItem(final Item item) {
        ...
        chr
            ...
            .getClient()
            .getSession()
            .write(InteractionPackets.GetTradeItemAdd((byte) 0, item));
        if (partner != null) {
            partner
                .getChr()
                .getClient()
                .getSession()
                .write(InteractionPackets.GetTradeItemAdd((byte) 1, item));
        }
    }
    ...
    public final boolean setItems(
        ...
        if (
            target == -1 ||
            ItemConstants.isPet(item.getItemId()) ||
            isLocked() ||
            (
    ...
    private int check() {
        ...
        if (
            chr.get().getInventory(InventoryType.EQUIP).getNumFreeSlot() < eq ||
            chr.get().getInventory(InventoryType.USE).getNumFreeSlot() < use ||
            chr.get().getInventory(InventoryType.SETUP).getNumFreeSlot() <
            setup ||
            chr.get().getInventory(InventoryType.ETC).getNumFreeSlot() < etc ||
            chr.get().getInventory(InventoryType.CASH).getNumFreeSlot() < cash
        ) {
            return 1;
        }
        return 0;
    ...
    public static final void completeTrade(final Player p) {
        ...
        partner
            ...
            .getClient()
            .getSession()
            .write(InteractionPackets.GetTradeConfirmation());
    ...
    public static final void cancelTrade(
        final Trade Localtrade,
        final Player p
    ) {
        Localtrade.cancel(p);
    ...
    public static final void startTrade(final Player c) {
        if (c.getTrade() == null) {
            ...
            c
                ...
                .getSession()
                .write(
                    InteractionPackets.GetTradeStart(
                        c.getClient(),
                        c.getTrade(),
                        (byte) 0
                    )
            ...
            c
                ...
                .getSession()
                .write(
                    PacketCreator.ServerNotice(5, "You are already in a trade")
                );
        }
    }
    ...
    public static final void inviteTrade(final Player c1, final Player c2) {
        ...
        if (c2 != null && c2.getTrade() == null) {
            ...
            c2
                .getClient()
                .getSession()
                .write(InteractionPackets.GetTradeInvite(c1));
        } else {
            c1
                .getClient()
                .getSession()
                .write(
                    PacketCreator.ServerNotice(
                        5,
                        "The other player is already trading with someone else."
                    )
                );
    ...
    public static final void visitTrade(final Player c1, final Player c2) {
        if (
            ...
            c2
                .getClient()
                .getSession()
                .write(InteractionPackets.GetTradePartnerAdd(c1));
            c1
                .getClient()
                .getSession()
                .write(
        ...
        if (trade != null) {
            if (trade.getPartner() != null) {
                ...
                if (other != null && other.getTrade() != null) {
                    ...
                    other.dropMessage(
                        5,
                        c.getName() + " has declined your trade request."
                    );
                }
            }
            trade.cancel(c);
