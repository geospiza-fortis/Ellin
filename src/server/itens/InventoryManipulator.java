...
public class InventoryManipulator {
    ...
    public static short addbyItem(
        final Client c,
        final Item item,
        final boolean fromcs
    ) {
        final InventoryType type = ItemConstants.getInventoryType(
            item.getItemId()
        );
    ...
    public static void addRing(
        ...
        if (
            ring == null ||
            ring.getUniqueId() != ringId ||
            ring.getUniqueId() <= 0 ||
            ring.getItemId() != itemId
        ) {
            return;
        }
    ...
    public static boolean addFromDrop(
        ...
        if (!type.equals(InventoryType.EQUIP)) {
            ...
            List<Item> existing = c
                .getPlayer()
                .getInventory(type)
                .listById(item.getItemId());
            if (
                    ...
                    while (quantity > 0) {
                        if (i.hasNext()) {
                            ...
                            if (
                                oldQ < slotMax &&
                                (
                                    eItem.getOwner().equals(eItem.getOwner()) ||
                                    eItem.getOwner() == null
                                )
                            ) {
                                short newQ = (short) Math.min(
                                    oldQ + quantity,
                                    slotMax
                                );
                                quantity -= (newQ - oldQ);
            ...
            while (
                ...
                if (newQ != 0) {
                    ...
                    if (newSlot == -1) {
                        ...
                        c
                            .getSession()
                            .write(PacketCreator.GetShowInventoryFull());
                    ...
                    c
                        .getSession()
                        .write(
                            PacketCreator.AddInventorySlot(type, nItem, true)
                        );
                    if (
                        (
                            ItemConstants.isThrowingStar(item.getItemId()) ||
            ...
            if (quantity == 1) {
                ...
                c
                    .getSession()
                    .write(PacketCreator.AddInventorySlot(type, item, true));
            } else {
                FileLogger.printError(
                    FileLogger.ITEM,
                    "Tried to pickup Equip id " +
                    item.getItemId() +
                    " containing more than 1 quantity --> " +
                    quantity
                );
    ...
    public static boolean addById(
        ...
        if (!type.equals(InventoryType.EQUIP)) {
                ...
                while (
                    ...
                    if (newQ != 0) {
                        ...
                        if (newSlot == -1) {
                            c
                                .getSession()
                                .write(PacketCreator.GetInventoryFull());
                            c
                                .getSession()
                                .write(PacketCreator.GetShowInventoryFull());
                            return false;
                        ...
                        c
                            .getSession()
                            .write(PacketCreator.AddInventorySlot(type, nItem));
                        if (
                            (
                                ItemConstants.isThrowingStar(itemId) ||
                                ItemConstants.isBullet(itemId)
            ...
            if (quantity == 1) {
                ...
                c
                    .getSession()
                    .write(PacketCreator.AddInventorySlot(type, nEquip));
            } else {
                throw new InventoryException(
                    "Trying to create equip with non-one quantity"
                );
            }
        }
        return true;
    ...
    public static boolean checkSpace(
        ...
        if (
            c == null ||
            c.getPlayer() == null ||
            c.getPlayer().getInventory(type) == null
        ) {
            return false;
        }
        if (!type.equals(InventoryType.EQUIP)) {
                ...
                if (existing.size() > 0) {
                    for (Item eItem : existing) {
                        ...
                        if (
                            ...
                            final short newQ = (short) Math.min(
                                oldQ + quantity,
                                slotMax
                            );
                            quantity -= (newQ - oldQ);
            ...
            if (slotMax > 0) {
                numSlotsNeeded =
                    (int) (Math.ceil(((double) quantity) / slotMax));
            } else {
                numSlotsNeeded = 1;
            }
    ...
    public static boolean removeFromSlot(
        Client c,
        InventoryType type,
        short slot,
        short quantity,
        ...
        if (item != null) {
            ...
            c
                .getPlayer()
                .getInventory(type)
                .removeItem(slot, quantity, allowZero);
    ...
    public static void move(
        ...
        if (
            src < 0 ||
            dst < 0 ||
            dst > c.getPlayer().getInventory(type).getSlotLimit() ||
            src == dst
        ) {
            return;
        }
        ...
        if (
            ...
            if ((olddstQ + oldsrcQ) > slotMax) {
                c
                    .getSession()
                    .write(
                        PacketCreator.MoveAndMergeWithRestInventoryItem(
                            type,
                            src,
                            dst,
                            (short) ((olddstQ + oldsrcQ) - slotMax),
                            slotMax
                        )
                    );
            } else {
                c
                    .getSession()
                    .write(
                        PacketCreator.MoveAndMergeInventoryItem(
                            type,
                            src,
                            dst,
                            (
            ...
            c
                .getSession()
                .write(PacketCreator.MoveInventoryItem(type, src, dst));
        }
    }
    ...
    public static void equip(Client c, short src, short dst) {
        ...
        boolean isCash = ItemInformationProvider
            .getInstance()
            .isCash(source.getItemId());
        SlotInformation slotCheck = SlotInformation.getFromItemId(
            source.getItemId()
        );
        ...
        if (
            ItemConstants.isWeapon(source.getItemId()) &&
            dst != ItemConstants.SHIELD &&
            dst != ItemConstants.WEAPON
        ) {
            AutobanManager
                .getInstance()
                .autoban(
                    c,
                    "Equipment hack, itemid " +
                    source.getItemId() +
                    " to slot " +
                    dst
                );
        ...
        switch (dst) {
            case -6:
                {
                    ...
                    if (
                        top != null && ItemConstants.isOverall(top.getItemId())
                    ) {
                        if (eqpdInv.isFull()) {
                            c
                                .getSession()
            ...
            case -5:
                {
                    ...
                    if (
                        top != null &&
                        ItemConstants.isOverall(source.getItemId())
                    ) {
                        if (
                            eqpInv.isFull(
                                bottom != null &&
            ...
            case -10:
                ...
                if (
                    weapon != null &&
                    ItemConstants.isTwoHanded(weapon.getItemId())
                ) {
                    if (eqpInv.isFull()) {
                        c.getSession().write(PacketCreator.GetInventoryFull());
                        c
            ...
            case -11:
                ...
                if (
                    shield != null &&
                    ItemConstants.isTwoHanded(source.getItemId())
                ) {
                    if (eqpInv.isFull()) {
                        c.getSession().write(PacketCreator.GetInventoryFull());
                        c
        ...
        if (
            p.getBuffedValue(BuffStat.BOOSTER) != null &&
        ...
        c
            .getSession()
            .write(
                PacketCreator.MoveInventoryItem(
                    InventoryType.EQUIP,
                    src,
                    dst,
                    (byte) 2
    ...
    public static void unequip(Client c, short src, short dst) {
        Equip source = (Equip) c
            .getPlayer()
            .getInventory(InventoryType.EQUIPPED)
            .getItem(src);
        Equip target = (Equip) c
            .getPlayer()
    ...
    public static void drop(
        ...
        if (
            ...
            source == null ||
            (quantity == 0 && !ItemConstants.isRechargeable(source.getItemId()))
        ) {
            c.getSession().write(PacketCreator.EnableActions());
            return;
        }
        ...
        if (
            c.getPlayer().getItemEffect() == itemId && source.getQuantity() == 1
        ) {
            ...
            c
                ...
                .getMap()
                .broadcastMessage(
                    EffectPackets.ItemEffect(c.getPlayer().getId(), 0)
                );
        }
        ...
        if (
            ...
            c
                .getSession()
                .write(PacketCreator.DropInventoryItemUpdate(type, source));
            ...
            c
                .getSession()
                .write(
                    PacketCreator.DropInventoryItem(
                        (src < 0 ? InventoryType.EQUIP : type),
                        src
                    )
                );
            if (src < 0) {
    ...
    public static void removeAllById(
        Client c,
        int itemId,
    ...
    public static int checkSpaceProgressively(
        Client c,
        int itemid,
        int quantity,
        String owner,
        ...
        if (
            ...
            haveItemWithId(c.getPlayer().getInventory(type), itemid)
        ) {
            return 0;
        }
            ...
            returnValue +=
                (
                        ...
                        !c
                            .getPlayer()
                            .getInventory(type)
                            .isFullAfterSomeItems(numSlotsNeeded - 1, usedSlots)
                    )
                    ? 1
                    : 0;
        } else {
            ...
            returnValue +=
                (
                        !c
                            .getPlayer()
                            .getInventory(type)
                            .isFullAfterSomeItems(0, usedSlots)
                    )
                    ? 1
                    : 0;
        }
