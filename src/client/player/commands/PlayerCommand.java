    ...
    public static class JoinEvent extends CommandExecute {
        ...
        public boolean execute(Client c, String[] splitted) {
            ...
            if (p.getSavedLocation(SavedLocationType.EVENT) > 1) {
                p.dropMessage(
                    "You can not use the command 2 times, use @leaveevent and enter again!"
                );
                return false;
            } else if (
                ...
                p.getMap().getId() ==
                p.getClient().getChannelServer().getEventMap()
            ) {
    ...
    public static class LeaveEvent extends CommandExecute {
        ...
        public boolean execute(Client c, String[] splitted) {
            ...
            if (
                ...
                p.dropMessage(
                    "There are no active events at the moment or there is no saved map. Please try again later!"
                );
                return false;
            }
        }
    ...
    public static class Playtime extends CommandExecute {
        ...
        public boolean execute(Client c, String[] splitted) {
            c
                .getPlayer()
                .dropMessage(
                    "Your playing time is currently " +
                    PlayerStringUtil.getTime(c.getPlayer().getPlaytime()) +
                    "."
                );
            return false;
        }
    ...
    public static class Gm extends CommandExecute {
        ...
        public boolean execute(Client c, String[] splitted) {
            ...
            for (int i = 0; i < CommandConstants.HELP_TYPES.length; i++) {
                if (
                    subString.contains(
                        CommandConstants.HELP_TYPES[i].toLowerCase()
                    )
                ) {
                    if (null != CommandConstants.HELP_TYPES[i]) {
                        CommandConstants.HelpCommandGM(
                            CommandConstants.HELP_TYPES[i],
                            p
                        );
                    }
                }
            ...
            if (!p.getCheatTracker().Spam(300000, 0)) {
                ...
                p.dropMessage(
                    "You can only send messages to GM every 5 minutes."
                );
                return false;
            }
        }
