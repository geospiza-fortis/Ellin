...
public class PlayerSkillFactory {
    ...
    private static final MapleData stringData = MapleDataProviderFactory
        .getDataProvider(new File(System.getProperty("wzpath") + "/String"))
        .getData("Skill.img");

    private static final ReentrantReadWriteLock locks = new ReentrantReadWriteLock();
    private static final ReadLock readLock = locks.readLock();
    ...
    public static final PlayerSkill getSkill(final int id) {
        ...
        final MapleDataProvider datasource = MapleDataProviderFactory.getDataProvider(
                    ...
                    if (data.getName().equals("skill")) {
                        for (MapleData dataTwo : data) {
                            if (dataTwo != null) {
                                ...
                                skills.put(
                                    skillid,
                                    PlayerSkill.loadFromData(skillid, dataTwo)
                                );
                                summonData =
                                    dataTwo.getChildByPath(
                                        "summon/attack1/info"
                                    );
                                if (summonData != null) {
                                    skillEntry = new PlayerSummonSkillEntry();
                                    skillEntry.attackAfter =
    ...
    public static String getSkillName(int id) {
        ...
        if (skillroot != null) {
            return MapleDataTool.getString(
                skillroot.getChildByPath("name"),
                ""
            );
        }
        return null;
    ...
    public static final PlayerSummonSkillEntry getSummonData(
        final int skillid
    ) {
