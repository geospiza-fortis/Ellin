    ...
    if (mode == -1) {
        ...
        if (status == 0) {
            ...
            if (qm.getPlayer().getHp() >= 50) {
                ...
                qm.getPlayer().updateSingleStat(
                    net.sf.odinms.client.MapleStat.HP,
                    25
                );
            }
    ...
    if (mode == -1) {
        ...
        if (status == 0) {
            if (qm.getPlayer().getHp() <= 0) {
                ...
                qm.sendNext(
                    "Hmmm... wait, this is...!! Wow... interesting... it's definitely worth studying. Hey, you look like a traveler... have you ever been to Eos Tower? If so, then you may have met a monster that resembles an Octopus, #o3230302#. A monster called #b#o3230302##k, isn't it strange?"
                );
            }
        } else if (status == 1) {
            qm.sendNextPrev(
                "What do I mean by this? Well, the way #o3230302# is structured, it really looks familiar...and after some researching, I have come to conclusion that it may be close to that of the aliens. Maybe #o3230302# is made by the aliens. It's defintely worth more attention. Can you help me on this?"
            );
        } else if (status == 2) {
            qm.sendNextPrev(
                "But you know... there's gotta be a better method to research something like #t4000099#. Something like a #bManual#k... but it&apos;s really hard to get, so just disregard what I just said."
            );
        } else if (status == 3) {
            qm.gainExp(8000);
