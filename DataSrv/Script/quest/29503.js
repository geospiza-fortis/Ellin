...
function start(mode, type, selection) {
    if (mode == 0) {
        if (status == 0) {
            qm.sendNext(
                "Come back when you feel like you are fully prepared for this."
            );
            qm.dispose();
            return;
        } else if (status == 2) {
    ...
    if (status == 0) {
        ...
        qm.sendNextPrev(
            "Best of luck to you. There's no real set date for this, so if you feel like you qualify for this, then feel free to come see me so I can determine whether you qualify for it. And remember that you will not be able to challenge other Titles unless you either forfeit this challenge or complete it..."
        );
        qm.dispose();
    }
}
