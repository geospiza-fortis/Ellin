...
function start() {
    if (cm.isQuestStarted(6192)) {
        if (cm.getWarpMap(921100300).getCharacters().size() > 0)
            cm.sendOk(
                "There is someone currently in this map, come back later."
            );
        else {
    ...
    if (
        !(cm.getPlayer().getLevel() >= 70 && parseInt(cm.getJobId() / 100) == 1)
    ) {
...
function action(mode, type, selection) {
    ...
    if (actionx["Mental"]) {
        if (status == 0)
            ...
            cm.sendYesNo(
                "Okay! Now, you'll be transformed into a much more powerful warrior through me. Before doing that, though, please make sure your SP has been thoroughly used, You'll need to use up at least all of SP's gained until level 70 to make the 3rd job advancement. Oh, and since you have already chosen your path of the occupation by the 2nd job adv., you won't have to choose again for the 3rd job adv. Do you want to do it right now?"
            );
        else if (status == 2) {
        ...
        cm.sendNext(
            "Go, talk with #b#p1022000##k and bring me #b#t4031057##k."
        );
        cm.dispose();
