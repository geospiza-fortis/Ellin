    ...
    if (status == 0 && mode == 1) {
        ...
        if (selectedType == 0) {
            ...
            var arrows = new Array(
                ...
                "Bronze Arrow for Crossbow",
                "Steel Arrow for Bow",
                "Steel Arrow for Crossbow"
            );
        ...
        if (!complete)
            cm.sendOk(
                "Hold it, I can't finish that without all of the proper materials. Bring them first, then we'll talk."
            );
        else {
            if (mats instanceof Array) {
                for (var i = 0; i < mats.length; i++) {
            ...
            cm.sendOk(
                "All done. If you need anything else, you know where to find me."
            );
        }
        cm.dispose();
    }
