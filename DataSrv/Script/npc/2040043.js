...
function generateCombo() {
    ...
    while (countPicked < 5) {
        ...
        if (positions[picked] == 1)
            // Don't let it pick one its already picked.
            continue;
    ...
    for (var i = 0; i < positions.length; i++) {
        ...
        if (i != positions.length - 1) returnString += ",";
    }
        ...
        if (eim.getProperty(stage.toString() + "stageclear") != null) {
            ...
            if (eim.isEventLeader(cm.getPlayer())) {
                ...
                if (state == -1) {
                    // preamble
                    cm.sendOk(
                        "Hi. Welcome to the #bstage " +
                            stage +
                            "#k. In this stage, line up 5 member of your party above those boxes in order to form the right combination to unlock the next stage. Only one player should stay above a box desired to pertain the combination."
                    );
                    ...
                    for (var i = 0; i < party.size(); i++) {
                        for (var y = 0; y < map.getAreas().size(); y++) {
                            if (
                                map
                                    .getArea(y)
                                    .contains(party.get(i).getPosition())
                            ) {
                                playersOnCombo++;
                                objset[y] = 1;
                    ...
                    if (
                        playersOnCombo == 5 ||
                        cm.getPlayer().getAdministrativeLevel() > 1
                    ) {
                        var comboStr = eim.getProperty(
                            "stage" + stage + "combo"
                        );
                        if (comboStr == null) {
                        ...
                        cm.sendNext(
                            "It looks like you haven't found the 5 boxes just yet. Please think of a different combination of boxes. Only 5 are allowed to stand on boxes, and if you move it may not count as an answer, so please keep that in mind. Keep going!"
                        );
                        cm.dispose();
                    }
                }
                ...
                cm.sendNext(
                    "Please tell your #bParty-Leader#k to come talk to me."
                );
            }
        }
