...
function doLoserMap(mode, type, selection) {
    ...
    if (mode == -1) {
        ...
        if (status == 0) {
            cm.sendNext(
                "Unfortunately, you did not manage to win this round. Better luck next time!"
            );
        } else if (status == 1) {
            var points = cm.getPlayer().getMCPQParty().getTotalCP();
            var grade = getGrade(points);
            var letterGrade = "ABCDFEE"[grade];

            cm.sendNext(
                "Sorry for the loss or tie ... Yet, you have put up quite an good show! Your team work and battle skills were amazing! I'll be expecting the same effort next time!\r\n\r\nMonster Carnival Rank: #b" +
                    letterGrade +
                    "#k"
            );
        } else if (status == 2) {
...
function doWinnerMap(mode, type, selection) {
    ...
    if (mode == -1) {
        ...
        if (status == 0) {
            cm.sendNext(
                "Congratulations! You managed to defeat the enemy team!"
            );
        } else if (status == 1) {
            var points = cm.getPlayer().getMCPQParty().getTotalCP();
            var grade = getGrade(points);
            var letterGrade = "ABCDFEE"[grade];
            cm.sendNext(
                "Congratulations on your victory!!! That was an amazing performance! The opposing party couldn't do a thing! I'll be expecting the same effort next time!\r\n\r\nMonster Carnival Rank: #b" +
                    letterGrade +
                    "\r\n\r\n#k"
            );
        } else if (status == 2) {
...
function doTown(mode, type, selection) {
    if (mode == -1) {
        ...
        if (status == 0) {
            cm.sendSimple(
                "What would you like to do? If you have never participated in the Monster Carnival, you'll need to know a thing or two about it before joining.\r\n\r\n#b#L0#Go to the Monster Carnival Field#l\r\n#L1#Learn about the Monster Carnival#l\r\n#L2#Trade Maple Coin#l"
            );
        } else if (status == 1) {
            if (selection == 0) {
                if (
                    cm.getPlayer().getLevel() < MonsterCarnival.MIN_LEVEL ||
                    cm.getPlayer().getLevel() > MonsterCarnival.MAX_LEVEL
                ) {
                    cm.sendOk(
                        "You must be between level " +
                            MonsterCarnival.MIN_LEVEL +
                            " and level " +
                            MonsterCarnival.MAX_LEVEL +
                            " to enter."
                    );
                    cm.dispose();
                    return;
            ...
            if (store) {
                ...
                if (cm.haveItem(coinId, purchaseCost)) {
                    cm.sendYesNo(
                        "Are you sure you want to purchase #i" +
                            purchaseId +
                            "#? You will have #r#e" +
                            (cm.itemQuantity(coinId) - purchaseCost) +
                            " " +
                            coinIcon +
                            "##k#n remaining."
                    );
