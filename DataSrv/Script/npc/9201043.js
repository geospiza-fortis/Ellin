...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (status == 0) {
            cm.sendSimple(
                "Meu nome � Amos the Strong. O que voc� gostaria de fazer?\r\n#b#L0#Entrar no desafio de Amoria#l\r\n#L1#Trocar 10 chaves para um bilhete!#l\r\n#k"
            );
        } else if (status == 1 && selection == 0) {
            if (cm.haveItem(4031592, 1) && cm.getPlayer().isMarried() == 1) {
                cm.sendYesNo("Ent�o voc� gostaria de ir para #bEntrada#k?");
                MySelection = selection;
            } else {
                cm.sendOk(
                    "Voc� deve ter um bilhete de entrada para entrar, e precisa estar casado."
                );
                cm.dispose();
            }
        } else if (status == 1 && selection == 1) {
            if (cm.haveItem(4031593, 10)) {
                cm.sendYesNo("Ent�o, voc� gostaria de um Ticket ?");
                MySelection = selection;
            } else {
