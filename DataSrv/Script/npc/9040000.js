...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (status == 0) {
            ...
            cm.sendSimple(
                "#e#b<Guild Quest: Sharenian Ruins>\r\n#k#n" +
            ...
            if (selection == 0) {
                if (!cm.isGuildLeader()) {
                    cm.sendOk(
                        "Your guild master/jr.master must talk to me to register the guild for the guild quest."
                    );
                    cm.dispose();
                } else {
                    if (em.isQueueFull()) {
                        cm.sendOk(
                            "The queue on this channel is already full. Please be patient and try again after a while, or try on another channel."
                        );
                        cm.dispose();
                    } else {
                        ...
                        cm.sendYesNo(
                            (qsize > 0
                                ? "There is currently #r" +
                                  qsize +
                                  "#k guilds queued on. "
                                : "") +
                                "Do you wish for your guild to join this queue?"
                        );
                    }
                }
            } else if (selection == 1) {
                if (cm.getPlayer().getGuildId() > 0) {
                    ...
                    if (eim == null) {
                        cm.sendOk(
                            "Your guild is not currently on strategy time on this channel. Check again if your guild is currently planning a Guild Quest or, if so, the channel they are allotted on."
                        );
                    } else {
                        if (cm.isLeader()) {
                    ...
                    cm.sendOk(
                        "You can't participate in the guild quest if you don't pertain on a guild yourself!"
                    );
                }
                ...
                reqStr += "\r\n\r\n    Team requirements:\r\n\r\n";
                reqStr +=
                    "     - 1 team member #rbelow or equal level 30#k.\r\n";
                reqStr +=
                    "     - 1 team member who is a #rThief with Dark Sight#k skill and #rmaxed Haste#k.\r\n";
                reqStr +=
                    "     - 1 team member who is a Magician with #rmaxed Teleport#k.\r\n";
                reqStr +=
                    "     - 1 team member who is a #rlong ranged attacker#k like Bowman, Assassin, or Gunslinger.\r\n";
                reqStr +=
                    "     - 1 team member with #rgood jumping skills#k like Assassin with maxed Flash Jump or Gunslinger with Wings.\r\n";

                cm.sendOk(
                    "#e#b<Guild Quest: Sharenian Ruins>#k#n\r\n Team up with your guild members in an auspicious attempt to recover the Rubian from the skeleton's grasp, with teamwork overcoming many puzzles and challenges awaiting inside the Sharenian tombs. Great rewards can be obtained upon the instance completion, and Guild Points can be racked up for your Guild." +
                        reqStr
                );
                cm.dispose();
            }
            ...
            if (sel == 0) {
                ...
                if (entry > 0) {
                    ...
                    cm.sendOk(
                        "Your guild is already queued on a channel. Please wait for your guild's turn."
                    );
                }
            }
