...
function start() {
    ...
    if (cm.isQuestStarted(2054)) zones = 3;
    else if (cm.isQuestStarted(2053)) zones = 2;
    else if (cm.isQuestStarted(2052)) zones = 1;
    else zones = 0;
}
...
function action(mode, type, selection) {
    if (mode == -1) cm.dispose();
        ...
        if (status == 1) {
            if (zones == 0) cm.dispose();
            else {
                var selStr =
                    "Its power allows you to will yourself deep inside the forest.#b";
