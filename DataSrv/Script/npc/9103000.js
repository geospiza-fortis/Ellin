    ...
    if (mode == -1) {
        ...
        if (status == 0) {
            if (!cm.isLeader()) {
                // not party leader
                cm.sendNext(
                    "Por favor, informe o seu #bLider do Grupo#k para falar comigo."
                );
                cm.dispose();
            } else {
                //party leader
                ...
                if (copns < 30) {
                    cm.sendNext(
                        "Voce deve ter 30 ou mais cupons para concluir a quest."
                    );
                    cm.dispose();
                } else {
                    cm.sendNext(
                        "Uau, voce recolheu " +
                            copns +
                            " cupons! Parabens! Voce sera levado para outro mapa para receber sua #bEXP#k e Rolly lhe dara as recompensas!"
                    );
                }
            }
        } else if (status == 1) {
