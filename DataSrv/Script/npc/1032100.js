    ...
    if (mode == -1) {
        ...
        if (status == 1 && mode == 0) {
            ...
            cm.sendNext(
                "It's not easy making " +
                    item +
                    ". Please get the materials ready."
            );
            cm.dispose();
            return;
        }
        ...
        if (status == 0) {
            if (cm.getLevel() >= 40) {
                ...
                cm.sendOk(
                    "I can make rare, valuable items but unfortunately I can't make it to a stranger like you."
                );
                cm.dispose();
            }
        } else if (status == 1) {
            cm.sendSimple(
                "What do you want to make?#b\r\n#L0#Moon Rock#l\r\n#L1#Star Rock#l\r\n#L2#Black Feather#l"
            );
        } else if (status == 2) {
            selected = selection;
            if (selection == 0) {
            ...
            if (selected == 0) {
                if (
                    ...
                    cm.sendNext(
                        "Ok here, take " +
                            item +
                            ". It's well-made, probably because I'm using good materials. If you need my help down the road, feel free to come back."
                    );
                } else {
                    cm.sendNext(
                        "Are you sure you have enough mesos? Please check and see if you have the refined #bBronze Plate#k, #bSteel Plate#k,\r\n#bMithril Plate#k, #bAdamantium Plate#k, #bSilver Plate#k, #bOrihalcon Plate#k and #bGold Plate#k, one of each."
                    );
                }
            } else if (selected == 1) {
                if (
                    ...
                    cm.sendNext(
                        "Are you sure you have enough mesos? Please check and see if you have the refined #bGarnet#k, #bAmethyst#k, #bAquaMarine#k, #bEmerald#k, #bOpal#k, #bSapphire#k, #bTopaz#k, #bDiamond#k and #bBlack Crystal#k, one of each."
                    );
                }
            } else if (selected == 2) {
                if (
