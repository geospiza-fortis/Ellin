...
function action(mode, type, selection) {
    ...
    if (mode == -1) {
        if (mode == 0)
            cm.sendNext(
                "This town also has a lot to offer. Find us if and when you feel the need to go to the Ant Tunnel Park."
            );
        cm.dispose();
        return;
    }
    if (status == 1) {
        ...
        if (cm.getMeso() < cost)
            cm.sendNext(
                "It looks like you don't have enough mesos. Sorry but you won't be able to use this without it."
            );
        else {
            cm.gainMeso(-cost);
            cm.warp(105070001);
