...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (
            eim.getProperty("stage4clear") != null &&
            eim.getProperty("stage4clear").equals("true")
        ) {
            cm.sendOk(
        ...
        if (status == 0) {
            if (cm.isEventLeader()) {
                cm.sendNext(
                    "After what I thought would be an immortal sleep, I have finally found someone that will save Sharenian. This old man will now pave the way for you to finish the quest."
                );
