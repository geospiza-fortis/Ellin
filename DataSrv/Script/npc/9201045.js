...
function start() {
    ...
    if (mapId == 670010500) curMap = 4;
    else if (mapId == 670010600) curMap = 5;
    else if (mapId == 670010700) curMap = 6;
        ...
        if (curMap == 4) {
            ...
            if (playerStatus) {
                ...
                if (status == 0) {
                    ...
                    if (preamble == null) {
                        cm.sendNext(
                            "Hello and welcome to the 5th stage. In this map, there are Toy trojans, Curse Eyes and Zombie Lupins. All you have to do is kill them and collect 50 #bCupid code pieces#k. Good Luck!"
                        );
                        ...
                        if (complete != null) {
                            cm.sendNext(
                                "Hey, why don't you continue? The portal opened!"
                            );
                            cm.dispose();
                        } else {
                            if (cm.haveItem(4031597, 50) == false) {
                                cm.sendNext(
                                    "I'm sorry, but you need a 50 #bCupid code pieces#k to clear this stage."
                                );
                                cm.dispose();
                            } else {
                                cm.sendNext(
                                    "Congratulations on clearing the fifth stage! I will now open the door."
                                );
                                clear(4, eim, cm);
                ...
                var eim = cm.getPlayer().getEventInstance();
                pstring =
                    "member5thpreamble" + cm.getPlayer().getId().toString();
                preamble = eim.getProperty(pstring);
                if (status == 0 && preamble == null) {
                    var qstring =
                        "member5th" + cm.getPlayer().getId().toString();
                    ...
                    if (complete != null) {
                        ...
                        cm.sendOk(
                            "Please talk to me after you've completed the stage."
                        );
                        cm.dispose();
                    }
                } else if (status == 1) {
            ...
            if (playerStatus) {
                if (eim.getProperty("rogSpawned") == null) {
                    if (status == 0) {
                        cm.sendNext(
                            "Wow, this is it... Your final mission. Kill Giest Balrog and bring back the #bGiest Fang#k he possesses. Good Luck!"
                        );
                    } else if (status == 1) {
                    ...
                    if (status == 0) {
                        if (cm.haveItem(4031594, 1)) {
                            cm.sendNext(
                                "You finished the challenge! Well, good luck in the vault..."
                            );
                        } else {
                            cm.sendNext(
                                "Please bring back the #bGiest Fang#k as evidence."
                            );
                            cm.dispose();
                        }
                    } else if (status == 1) {
