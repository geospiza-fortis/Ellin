...
function start() {
    if (
        !(cm.getPlayer().getLevel() >= 70 && parseInt(cm.getJobId() / 100) == 3)
    ) {
...
function action(mode, type, selection) {
    ...
    if (actionx["Mental"]) {
        ...
        cm.sendNext(
            "Go, talk with #b#p1012100##k and bring me #b#t4031057##k."
        );
        cm.dispose();
        ...
        if (sel == 0) {
            if (cm.getPlayer().getLevel() >= 70 && cm.getJobId() % 10 == 0) {
                if (status == 0)
                    ...
                    cm.sendNextPrev(
                        "The mental half of the test can only start after you pass the physical part of the test. #b#t4031057##k will be the proof that you have indeed passed the test. I'll let #b#p1012100##k in advance that you're making your way there, so get ready. It won't be easy, but I have the utmost faith in you. Good luck."
                    );
            }
        } else {
