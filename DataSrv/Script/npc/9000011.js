...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (status == 0) {
            cm.sendNext(
                "Hey, I'm #bMartin#k. I am waiting for my brothers... What takes them so long? I got bored now... If we do not get there on time, we might not be able to participate in the event..."
            );
        } else if (status == 1) {
            cm.sendNextPrev(
                "Hmm... What should I do? The event will start, soon... Many people went to participate in the event, so we better be hurry..."
            );
        } else if (status == 2) {
            cm.sendSimple(
                "Hey... Why don't you go with me, then?\r\n#L0##e1.#n#b What kind of an event is it?#k#l\r\n#L1##e2.#n#b Explain the event game to me.#k#l\r\n#L2##e3.#n#b Alright, let's go!#k#l"
            );
        } else if (status == 3) {
            if (selection == 0) {
                cm.sendNext(
