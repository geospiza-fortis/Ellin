...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (status == 0) {
            cm.sendNext(
                "Welcome. We're the Sleepywood Hotel. Our hotel works hard to serve you the best at all times. If you are tired and worn out from hunting, how about a relaxing stay at our hotel?"
            );
        }
        if (status == 1) {
            cm.sendSimple(
        ...
        if (status == 3) {
            if (tempvar == 0) {
                if (cm.getPlayer().getMeso() >= regcost) {
                    ...
                    cm.sendNext(
                        "I'm sorry. It looks like you don't have enough mesos. It will cost you at least " +
                            regcost +
                            "mesos to stay at our hotel."
                    );
                }
            }
            if (tempvar == 1) {
