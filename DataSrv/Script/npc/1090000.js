        ...
        if (status == 0) {
            if (cm.getJob().equals(MapleJob.BEGINNER)) {
                if (cm.getLevel() >= 10)
                    ...
                    cm.sendOk(
                        "Train a bit more and I can show you the way of the #rPirate#k."
                    );
                    cm.dispose();
                }
            } else {
                if (
                    ...
                    (cm.getJob().equals(MapleJob.GUNSLINGER) ||
                        cm.getJob().equals(MapleJob.BRAWLER))
                ) {
                    cm.sendOk(
                        "Please go visit #bPedro#k. He resides in #bEl Nath#k."
                    );
                    cm.dispose();
                } else if (
                    cm.getLevel() < 30 &&
                    ...
                    cm.sendOk(
                        "Please come back to see me once you have trained more."
                    );
                    cm.dispose();
                } else if (
                    cm.getLevel() >= 120 &&
                    (cm.getJob().equals(MapleJob.OUTLAW) ||
                        cm.getJob().equals(MapleJob.MARAUDER))
                ) {
                    cm.sendOk(
                        "Please go visit #bSamuel#k. He resides in #bLeafre#k."
                    );
                    cm.dispose();
                } else {
                    cm.sendOk(
                        "You are not a #rPirate#k, I have no business with you."
                    );
                    cm.dispose();
                }
                111;
            ...
            if (cm.getJob().equals(MapleJob.BEGINNER)) {
                ...
                cm.getPlayer().updateSingleStat(
                    MapleStat.AVAILABLEAP,
                    33,
                    false
                );
            ...
            cm.sendNextPrev(
                "You are now ready to take the next step as a #rBrawler#k or #rGunslinger#k."
            );
        } else if (status == 12) {
            cm.sendSimple(
