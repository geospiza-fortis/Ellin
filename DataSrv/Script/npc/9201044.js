...
var stage3combos = Array(
    Array(0, 0, 0, 0, 1, 1, 1, 1, 1), //Lol, it took me forever to work these out :D
...
function start() {
    ...
    if (mapId == 670010200) curMap = 1;
    else if (mapId == 670010300) curMap = 2;
    else if (mapId == 670010400) curMap = 3;
        ...
        if (curMap == 1) {
            ...
            if (playerStatus) {
                ...
                if (status == 0) {
                    ...
                    if (preamble == null) {
                        cm.sendNext(
                            "Hello and welcome to the first stage. I will spawn 3 portals here, and you will decide which one is the right one, girls will go into the left portal, while guys go into the right portal. Kill the monsters there and then get the leader to talk to my friend, Glimmer Man."
                        );
                        var map = eim.getMapInstance(cm.getPlayer().getMapId());
                        var packetglow = MaplePacketCreator.environmentChange(
                            "gate",
                            2
                        );
                        ...
                        var complete = eim.getProperty(
                            curMap.toString() + "stageclear"
                        );
                        if (complete != null) {
                            cm.warp(670010300, 0);
                            cm.dispose();
                            ...
                            if (cm.haveItem(4031595, 1) == false) {
                                cm.sendNext(
                                    "I'm sorry, but you need a #bPiece of Broken Mirror#k to clear this stage."
                                );
                                cm.dispose();
                            } else {
                                cm.sendNext(
                                    "Congratulations on clearing the first stage! Tell your party members to click on me to continue."
                                );
                                clear(1, eim, cm);
    ...
    if (curMap == 2) {
        if (playerStatus) {
            ...
            if (status == 0) {
                ...
                if (preamble == null) {
                    cm.sendNext(
                        "Hi. Welcome to the 2nd stage. To clear this stage, all you have to do, is have 5 people hang on the ropes. If you get the right amount of people hanging on each rope, the weight will cause the gate to open."
                    );
                    eim.setProperty("leader" + nthtext + "preamble", "done");
                    var sequenceNum = Math.floor(
                        Math.random() * curcombo.length
                    );
                    eim.setProperty(
                        "stage" + nthtext + "combo",
                    ...
                    // check for people on ropes
                        ...
                        for (i = 0; i < objset.length; i++) {
                            for (j = 0; j < party.size(); j++) {
                                var present = currect
                                    .get(i)
                                    .contains(party.get(j).getPosition());
                                if (present) {
                                    objset[i] = objset[i] + 1;
                        ...
                        if (totplayers == 5 /*|| debug*/) {
                            ...
                            if (testcombo || debug) {
                                ...
                                // do wrong
                                failstage(eim, cm);
                                var tries = Integer.parseInt(
                                    eim.getProperty("stage2Tries")
                                );
                                eim.setProperty("stage2Tries", tries + 1);
                                if (tries == 7) {
                                    cm.sendOk(
                                        "You got the combination wrong seven times, so now it will be reset."
                                    );
                                    var sequenceNum = Math.floor(
                                        Math.random() * curcombo.length
                                    );
                            ...
                            if (debug) {
                                ...
                                cm.sendNext(
                                    "Looks like you haven't found the ropes just yet. Keep trying!!"
                                );
                            //var combo = curcombo[parseInt(eim.getProperty("stage" + nthtext + "combo"))];
                            //cm.sendOk(combo);
                            //clear(curMap,eim,cm);
                    ...
                    // check for people on ropes
                        ...
                        if (totplayers == 5 /*|| debug*/) {
                            ...
                            if (testcombo || debug) {
                                ...
                                if (tries == 7) {
                                    cm.sendOk(
                                        "The combination has been reset because you didn't get it right in 7 guesses."
                                    );
                                    var sequenceNum = Math.floor(
                                        Math.random() * curcombo.length
                                    );
                                ...
                                var randSlime11 = Math.floor(
                                    Math.random() * randomSlime.length
                                );
                                var randSlime = randomSlime[randSlime11];
                                for (var k = 0; k < numSpawn; k++)
                                    cm.spawnMonster(randSlime, 1706, 300);
                            ...
                            if (debug) {
                                ...
                                cm.sendNext(
                                    "It looks like you haven't found the 5 " +
                                        nthobj +
                                        " just yet. Please think of a different combination of " +
                                        nthobj +
                                        ". Only 5 are allowed to " +
                                        nthverb +
                                        " on " +
                                        nthobj +
                                        ", and if you " +
