        ...
        if (status == 0) {
            ...
            if (curStage == 0) {
                if (cm.haveItem(4001087, 1)) {
                    cm.sendNext(
                        "Great, you brought me the key. The door will now be unlocked."
                    );
                    eim.setProperty("2stageclear", "true");
                    eim.setProperty("theLvl", curStage + 1);
                    var packetef = MaplePacketCreator.showEffect(
                        "quest/party/clear"
                    );
                    var packetsnd = MaplePacketCreator.playSound(
                        "Party1/Clear"
                    );
            ...
            if (curStage == 4) {
                cm.sendNext(
                    "You have cleared the maze, wait for your party leader to appear."
                );
                cm.dispose();
            }
